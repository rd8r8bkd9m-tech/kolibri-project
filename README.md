...
- Настройте переменные окружения для `backend/service/main.py`:
  ...
  ```
- Для «форка» разных моделей запускайте скрипт `scripts/llm_teacher.py` (он запрашивает внешний LLM и обучает Kolibri через teach/feedback) с разными `--llm-model` и фиксируйте ответы в базе знаний.
