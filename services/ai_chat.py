...
def _sticky_session_title(
    ...
    if cid and conv:
        try:
            rows = _DB.list_conversation_sessions(cid, limit=500)
        except Exception:
            rows = []
        for item in rows:
            if str(item.get("conversation_id") or "") != conv:
                continue
            current_title = str(item.get("title") or "").strip()
            if current_title and current_title != "Новый чат":
                return current_title
            break
    return _session_title_from_message(message)
...
async def ai_chat(req: ChatRequest, request: Request) -> ChatResponse:
    ...
    if settings.response_mode == "llm":
        ...
        _upsert_conversation_session(
            client_key,
            conversation.id,
            title=_sticky_session_title(client_key, conversation.id, effective_req.message),
        )
    ...
    if settings.response_mode == "llm":
        async def llm_stream_generator():
            ...
            for i, word in enumerate(words):
                ...
                # await asyncio.sleep(0.01)  # Removed delay for faster streaming
