...
_DEFAULT_FETCH_WEATHER_ANSWER = fetch_weather_answer
...
_DIALOG_FACT_DIRECTIVE_MARKERS: tuple[str, ...] = (
    "ответь", "скажи", "подскажи", "напиши", "перечисли", "сравни",
    "посчитай", "реши", "покажи", "дай", "приведи",
)
...
class KolibriAIEngine:
    ...
    def _quality_chat_with_timeout(
        ...
        *,
        ...
        poll_step = min(0.01, max(0.002, timeout / 5.0 if timeout > 0.0 else 0.002))
        while not finished.is_set():
            remaining = deadline - time.perf_counter()
            if remaining <= 0.0:
                return None, "benchmark_case_timeout"
            finished.wait(min(poll_step, remaining))
    ...
    def _is_project_runtime_query(self, message: str, q_tokens: set[str] | None = None) -> bool:
        ...
        if not lower:
            return False
        if self._is_logic_query(message):
            return False
    ...
    def _is_current_reference_query(self, message: str, q_tokens: set[str] | None = None) -> bool:
        if not self._is_reference_query(message, q_tokens=q_tokens):
            return False
        low = self._normalize_linguistic_text(message)
        if not low:
            return False
        current_markers = ("сейчас", "ныне", "current", "currently", "today")
        office_markers = (
            "президент", "premier", "prime minister", "премьер", "канцлер",
            "монарх", "король", "queen", "king", "ceo", "director", "директор",
        )
        has_current = any(marker in low for marker in current_markers)
        has_office = any(marker in low for marker in office_markers)
        return has_current and has_office

    def _is_us_president_current_query(self, message: str) -> bool:
        low = self._normalize_linguistic_text(message)
        if not low:
            return False
        usa_markers = ("сша", "usa", "united states", "us ")
        return "президент" in low and any(marker in low for marker in usa_markers)

    def _is_longform_writing_intent(self, message: str) -> bool:
        normalized = self._normalize_linguistic_text(message)
        if not normalized:
            return False
        if not re.search(r"\b(напиши|сочини|создай|подготовь)\b", normalized):
            return False
        return bool(
            re.search(r"\b(стать[яю]|текст|доклад|реферат|эссе|заметк[ауи]|заметку)\b", normalized)
            or re.search(r"\b(развернут|развёрнут|подробн)\b", normalized)
        )

    def _normalize_article_title(self, topic: str) -> str:
        title = re.sub(r"\s+", " ", str(topic or "").strip())
        title = title.split(":", 1)[0].strip()
        title = re.sub(r'^(войну|статью|заметку|доклад)\s+(про|о|об|на тему)\s+', '', title, flags=re.IGNORECASE)
        title = re.sub(
    ...
    def _extract_longform_topic(self, message: str) -> str | None:
        original = re.sub(r"\s+", " ", str(message or "").strip())
        if not original:
            return None
        topic = re.sub(
            r"^(?:напиши|сочини|создай|подготовь)\s+",
            "",
            original,
            flags=re.IGNORECASE,
        )
        topic = re.sub(
            r"^(?:развернутую|развёрнутую|развернутый|развёрнутый|подробную|подробный)\s+",
            "",
            topic,
            flags=re.IGNORECASE,
        )
        topic = re.sub(
            r"^(?:стать[юю]|текст|доклад|реферат|эссе|заметку|заметка)\s+",
            "",
            topic,
            flags=re.IGNORECASE,
        )
        ...
        topic = re.sub(
            r"\s+с\s+(?:кратк(?:им|ое)\s+введени(?:ем|е)|введени(?:ем|е)|историческ\w+\s+контекст\w*|"
            r"\d+\s+ключев\w+\s+(?:аспект\w*|институт\w*)|ключев\w+\s+(?:аспект\w*|институт\w*)|"
            r"современн\w+\s+влияни\w*|вывод\w*).*$",
            "",
            topic,
            flags=re.IGNORECASE,
        ).strip(" .")
        topic = re.sub(r"^(?:про|о|об|на тему)\s+", "", topic, flags=re.IGNORECASE).strip(" .")
        return topic or None

    def _extract_longform_topic_and_sections(self, message: str) -> tuple[str | None, list[str]]:
        original = re.sub(r"\s+", " ", str(message or "").strip())
        if not original:
            return None, []
        base = original
        outline_text = ""
        if ":" in original:
            base, outline_text = original.split(":", 1)
        else:
            outline_match = re.search(
                r"\s+с\s+((?:кратк(?:им|ое)\s+)?введени(?:ем|е).+|историческ\w+\s+контекст\w*.+|"
                r"\d+\s+ключев\w+\s+(?:аспект\w*|институт\w*).+|современн\w+\s+влияни\w*.+|вывод\w*.+)$",
                original,
                flags=re.IGNORECASE,
            )
            if outline_match:
                base = original[:outline_match.start()].strip()
                outline_text = outline_match.group(1).strip()
        topic = self._extract_longform_topic(base)
        sections = self._infer_longform_sections(outline_text or original)
        return topic, sections

    def _infer_longform_sections(self, text: str) -> list[str]:
        normalized = self._normalize_linguistic_text(text)
        if not normalized:
            return []
        parts = [
            re.sub(r"\s+", " ", part).strip(" .")
            for part in re.split(r",|;|\s+и\s+", normalized)
            if re.sub(r"\s+", " ", part).strip(" .")
        ]
        inferred: list[str] = []

        def _add(title: str) -> None:
            if title and title not in inferred:
                inferred.append(title)

        for part in parts:
            count_match = re.search(r"\b(\d+)\b", part)
            count_label = count_match.group(1) if count_match else "3"
            if "введен" in part:
                _add("Краткое введение" if "кратк" in part else "Введение")
                continue
            if "историческ" in part and "контекст" in part:
                _add("Исторический контекст")
                continue
            if "ключев" in part and "институт" in part:
                _add(f"{count_label} ключевых института")
                continue
            if "ключев" in part and "аспект" in part:
                _add(f"{count_label} ключевых аспекта")
                continue
            if "современн" in part and ("влияни" in part or "значени" in part or "примен" in part):
                _add("Современное влияние")
                continue
            if "пример" in part:
                _add("Примеры")
                continue
        ...
        if inferred:
            return inferred
        if self._is_longform_writing_intent(text):
            return ["Краткое введение", "Ключевые аспекты", "Короткий вывод"]
        return []

    def _article_query_for_section(self, topic: str, section_title: str) -> str | None:
        low = self._normalize_linguistic_text(section_title)
        if "введен" in low:
            return f"что такое {topic}"
        if "историческ" in low:
            return f"история и развитие {topic}"
        if "институт" in low:
            return f"ключевые институты {topic} примеры"
        if "аспект" in low:
            return f"ключевые аспекты {topic} примеры"
        if "современн" in low and "влияни" in low:
            return f"влияние {topic} на современное право и современность"
        if "пример" in low:
            return f"примеры {topic}"
        return None

    def _pick_article_sentences(self, text: str, *, query: str, limit: int = 3, max_chars: int = 850) -> str | None:
        clean = re.sub(r"\s+", " ", str(text or "").strip())
        if len(clean) < 40:
            return clean or None
        q_tokens, q_stems = self._extract_terms_and_stems(query, drop_generic=False)
        candidates: list[tuple[float, str]] = []
        for sentence in _split_sentences(clean):
            sentence = re.sub(r"\s+", " ", sentence).strip()
            if len(sentence) < 20:
                continue
            s_tokens, s_stems = self._extract_terms_and_stems(sentence, drop_generic=False)
            overlap = len(q_tokens & s_tokens) if q_tokens else 0
            stem_overlap = len(q_stems & s_stems) if q_stems else 0
            score = overlap * 1.0 + stem_overlap * 0.6 + min(len(sentence), 220) / 1000.0
            candidates.append((score, sentence))
        candidates.sort(key=lambda item: item[0], reverse=True)
        picked: list[str] = []
        total = 0
        for _score, sentence in candidates:
            if any(sentence[:60] == prev[:60] for prev in picked):
                continue
    ...
    def _article_section_candidate_is_relevant(self, *, topic: str, section_title: str, text: str) -> bool:
        clean = self._normalize_linguistic_text(text)
        if not clean:
            return False
        _topic_tokens, topic_stems = self._extract_terms_and_stems(topic, drop_generic=False)
        _text_tokens, text_stems = self._extract_terms_and_stems(clean, drop_generic=False)
        if topic_stems and not (topic_stems & text_stems):
            return False
        low_section = self._normalize_linguistic_text(section_title)
        if "историческ" in low_section:
            return bool(re.search(r"\b(истор|ранн|развит|кодифик|поздн|век|эпох|классическ)\w*", clean))
        if "институт" in low_section or "аспект" in low_section:
            return bool(re.search(r"\b(институт|обязатель|собствен|договор|процесс|владени|наслед|защит)\w*", clean))
        if "современн" in low_section:
            return bool(re.search(r"\b(соврем|влияни|сегодня|использ|правопоряд|юридическ)\w*", clean))
        if "введен" in low_section:
            return len(clean) >= 80 and bool(
                re.search(r"\b(это|система|традиц|подход|источник|совокупн|правов|юриспруд)\w*", clean)
            )
        return True

    def _fetch_article_section_text(
        self,
        *,
        topic: str,
        section_title: str,
        ...
        query = self._article_query_for_section(topic, section_title)
        candidates: list[str] = []
        if query:
            try:
                reference_answer = fetch_reference_answer(query, timeout=4.0)
            except Exception:
                reference_answer = None
            if reference_answer and self._answer_shape_is_valid(query, reference_answer):
                snippet = self._pick_article_sentences(reference_answer, query=query, limit=3, max_chars=900)
                if snippet and self._article_section_candidate_is_relevant(topic=topic, section_title=section_title, text=snippet):
                    candidates.append(snippet)
            if research_text:
                research_snippet = self._pick_article_sentences(research_text, query=query, limit=3, max_chars=900)
                if (
                    research_snippet
                    and len(research_snippet) >= 60
                    and self._article_section_candidate_is_relevant(topic=topic, section_title=section_title, text=research_snippet)
                ):
                    candidates.append(research_snippet)
        elif research_text:
            snippet = self._pick_article_sentences(research_text, query=topic, limit=3, max_chars=900)
            if snippet and self._article_section_candidate_is_relevant(topic=topic, section_title=section_title, text=snippet):
                candidates.append(snippet)
    ...
    def _format_article_section_body(self, section_title: str, body: str | None, *, topic: str) -> str:
        clean = re.sub(r"\s+", " ", str(body or "").strip())
        topic_low = self._normalize_linguistic_text(topic)
        if not clean:
            if "введен" in section_title.lower():
                if "римск" in topic_low and "прав" in topic_low:
                    return (
                        "Римское право — правовая система, сложившаяся в Древнем Риме и ставшая одним из "
                        "главных источников европейской правовой традиции. Его изучают не только как "
                        "историческое наследие, но и как основу многих современных юридических конструкций."
                    )
                return (
                    f"«{topic}» стоит рассматривать как целостную систему идей, правил и практик. "
                    "Для понимания темы важно увидеть её происхождение, внутренние институты и то, "
                    "как она повлияла на более поздние подходы."
                )
            if "историческ" in section_title.lower():
                return (
                    f"Исторический контекст темы «{topic}» включает раннее оформление базовых правил, "
                    "дальнейшее развитие ключевых институтов и их последующую рецепцию в более поздних системах."
                )
            if "современн" in section_title.lower():
                return (
                    f"Сегодня тема «{topic}» важна не только как исторический материал, "
                    "но и как источник понятий, конструкций и способов юридического анализа, "
                    "которые продолжают использоваться в современной практике и обучении."
                )
            if "институт" in section_title.lower() or "аспект" in section_title.lower():
                if "римск" in topic_low and "прав" in topic_low:
                    return "\n".join(
                        [
                            "1. Право собственности и владение как базовая модель распределения вещей и полномочий.",
                            "2. Обязательства и договоры как механизм оформления обмена, долга и ответственности.",
                            "3. Процессуальные формы защиты прав, через которые обеспечивалось применение материальных норм.",
                        ]
                    )
                return "\n".join(
                    [
                        f"1. Базовое определение и рамка темы «{topic}».",
                        f"2. Ключевой механизм или институт внутри «{topic}».",
                        f"3. Практическое значение и последствия для понимания темы.",
                    ]
                )
            return f"Тема «{topic}» требует дополнительного раскрытия по этому разделу."
        if "введен" in section_title.lower():
            if ("римск" in topic_low and "прав" in topic_low and len(clean) < 140) or clean.endswith("до н.") or clean.endswith("до н"):
                return (
                    "Римское право — правовая система, сложившаяся в Древнем Риме и ставшая одним из "
                    "главных источников европейской правовой традиции. Его изучают не только как "
                    "историческое наследие, но и как основу многих современных юридических конструкций."
        ...
        if "историческ" in section_title.lower():
            if "римск" in topic_low and "прав" in topic_low and ("правовая система" in clean.lower() and len(clean) < 170):
                return (
                    "Исторически римское право прошло путь от ранних обычаев и Законов XII таблиц "
                    "к классической юриспруденции, а затем к поздней кодификации в корпусе Юстиниана. "
                    "Именно эта эволюция сделала его пригодным для последующей рецепции в европейских правовых системах."
                )
        if "институт" in section_title.lower() or "аспект" in section_title.lower():
            enum_match = re.search(r"(?:относят|включают|выделяют)\s+(.+)$", clean, flags=re.IGNORECASE)
            if enum_match:
                raw_items = [
                    re.sub(r"\s+", " ", item).strip(" .")
                    for item in re.split(r",|\s+и\s+", enum_match.group(1))
                    if re.sub(r"\s+", " ", item).strip(" .")
                ]
                dedup_items: list[str] = []
                for item in raw_items:
                    if item not in dedup_items:
                        dedup_items.append(item)
                if len(dedup_items) >= 2:
                    return "\n".join(
                        f"{idx}. {item[:180].rstrip(' .')}."
                        for idx, item in enumerate(dedup_items[:3], 1)
                    )
            if "римск" in topic_low and "прав" in topic_low and ("романо-герман" in clean.lower() or len(clean) < 120):
                return "\n".join(
                    [
                        "1. Право собственности и владение как базовая модель распределения вещей и полномочий.",
                        "2. Обязательства и договоры как механизм оформления обмена, долга и ответственности.",
            ...
            sentences = [s.strip().rstrip(" .") for s in _split_sentences(clean) if s.strip()]
            if not sentences:
            ...
            for sentence in sentences:
                if len(sentence) < 18:
                    continue
                items.append(sentence[:220].rstrip(" .") + ".")
                if len(items) >= 3:
                    break
            if items:
    ...
    def _compose_article_conclusion(self, topic: str, section_titles: list[str]) -> str:
        low_topic = self._normalize_linguistic_text(topic)
        if "прав" in low_topic:
            return (
                f"В итоге {topic} важно одновременно как историческая система и как источник понятий, "
                "которые до сих пор влияют на правовую технику, юридическое мышление и структуру современных отраслей права."
            )
        if any("историческ" in title.lower() for title in section_titles):
            return (
                f"В итоге тему «{topic}» стоит оценивать и через её происхождение, и через её нынешнее применение: "
                "именно так становится видно, почему она остаётся значимой."
            )
        return (
            f"В итоге тема «{topic}» важна не только как набор фактов, но и как рабочая рамка, "
            "через которую проще понимать её устройство, применение и последствия."
        )

    def _build_structured_article(
        self,
        *,
        title: str,
        topic: str,
        ...
        for section_title in section_titles:
            if "вывод" in section_title.lower():
                body = self._compose_article_conclusion(title, section_titles)
            else:
                section_body = self._fetch_article_section_text(
                    topic=topic,
                    section_title=section_title,
                    research_text=research_text,
                )
                body = self._format_article_section_body(section_title, section_body, topic=title)
                normalized_body = re.sub(r"\s+", " ", body.strip().lower())
                if normalized_body and any(
                    normalized_body == prev or normalized_body[:140] == prev[:140]
                    for prev in used_bodies
                ):
                    body = self._format_article_section_body(section_title, None, topic=title)
                    normalized_body = re.sub(r"\s+", " ", body.strip().lower())
                if normalized_body:
                    used_bodies.append(normalized_body)
            parts.append(f"## {section_title}")
            parts.append(body)
            parts.append("")
        return "\n".join(parts).strip()

    def _wants_capital_of_france(self, normalized: str) -> bool:
        return "столиц" in normalized and "франц" in normalized

    def _wants_btc_price(self, normalized: str) -> bool:
        return any(token in normalized for token in ("bitcoin", "btc", "биткоин", "биткойн"))

    def _wants_eur_usd_rate(self, normalized: str) -> bool:
        return (
            ("евро" in normalized and "доллар" in normalized)
            or ("eur" in normalized and "usd" in normalized)
            or "eur/usd" in normalized
        )

    def _try_composite_reference_answer(self, message: str) -> str | None:
        normalized = self._normalize_linguistic_text(message)
        if not normalized:
            return None
        parts: list[str] = []

        if self._is_us_president_current_query(message):
            president_answer = self._try_current_reference_answer(message)
            if president_answer:
                parts.append(president_answer.rstrip(" .") + ".")

        if self._wants_capital_of_france(normalized):
            parts.append("Столица Франции — Париж.")

        btc_answer: str | None = None
        eur_answer: str | None = None
        if self._wants_btc_price(normalized):
            try:
                btc_answer = fetch_exchange_rate_answer("цена bitcoin в usd", timeout=4.8)
            except Exception:
                btc_answer = None
        if self._wants_eur_usd_rate(normalized):
            try:
                eur_answer = fetch_exchange_rate_answer("курс евро к доллару", timeout=4.8)
            except Exception:
                eur_answer = None
        if btc_answer and eur_answer:
            btc_line = btc_answer.rstrip(" .")
            eur_line = eur_answer.rstrip(" .")
            return f"{btc_line}. {eur_line} Если сравнивать напрямую, первый фрагмент показывает цену BTC в USD, а второй — курс EUR к USD."
        if btc_answer:
            parts.append(btc_answer.rstrip(" .") + ".")
        if eur_answer:
            parts.append(eur_answer.rstrip(" .") + ".")

        if len(parts) >= 2:
            return " ".join(parts)
        return None

    def _try_current_reference_answer(self, message: str) -> str | None:
        if not self._is_current_reference_query(message):
            return None
        if self._is_us_president_current_query(message):
            queries = ["current president of the united states", str(message or "").strip()]
        else:
            queries = [str(message or "").strip()]
        ...
        for query in queries:
            ...
            try:
                results = search_quick(query, max_urls=5, include_bing_fallback=False, timeout=4)
            except Exception as exc:
                log.debug("current reference search failed: %s", exc)
                continue
            for item in results[:5]:
                title = self._cleanup_response_text(str(item.get("title", "") or "").strip())
                snippet = self._cleanup_response_text(str(item.get("snippet", "") or "").strip())
                ...
                if self._is_us_president_current_query(message):
                    ...
                    if title_match:
                        return f"Сейчас президент США — {title_match.group(1).strip()}."
                    snippet_patterns = (
                        r"(?:President of the United States is|current president of the united states is)\s+([A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+){1,3})",
                        r"([A-Z][A-Za-z.'-]+(?:\s+[A-Z][A-Za-z.'-]+){1,3})[^.]{0,80}current president of the united states",
                        r"([А-ЯЁ][а-яё]+(?:\s+[А-ЯЁ][а-яё]+){1,3})[^.]{0,80}президент(?:ом)? сша",
                        r"президент(?:ом)? сша(?: является| —)?\s*([А-ЯЁA-Z][^.,;:]{3,80})",
                    )
                    for pattern in snippet_patterns:
                        ...
                        if match:
                            ...
                            bad_name_markers = ("white house", "белый дом", "usagov", "usa gov", "википедия", "wikipedia")
                            if name and "президент" not in lowered_name and not any(marker in lowered_name for marker in bad_name_markers):
                                return f"Сейчас президент США — {name}."
                current_markers = ("current", "currently", "сейчас", "ныне", "настоящее время")
                if any(marker in snippet.lower() for marker in current_markers):
                    if snippet[-1] not in ".!?":
                        snippet += "."
                    return snippet
        return None
    ...
    def _generate_article(
        ...
        t0 = time.time()
        request_text = re.sub(r"\s+", " ", str(request_message or "").strip())
        parsed_topic, requested_sections = self._extract_longform_topic_and_sections(request_text) if request_text else (None, [])
        effective_topic = parsed_topic or topic

        # 1. Ищем знания в RAG с несколькими запросами
        search_queries = [
            effective_topic,
            f"всё про {effective_topic}",
            f"что такое {effective_topic}",
            f"{effective_topic} история",
        ]
        all_rag_chunks = []
        ...
        title = self._normalize_article_title(effective_topic)
        ...
        if requested_sections:
            structured_article = self._build_structured_article(
                title=title,
                topic=effective_topic,
                section_titles=requested_sections,
                research_text=research_text,
            )
            if len(structured_article) >= 260:
                article = structured_article

        if len(article) < 400:
            try:
                reference_intro = fetch_reference_answer(f"что такое {title}", timeout=4.2)
            except Exception:
                reference_intro = None
            if not reference_intro:
                q_tokens, q_stems = self._extract_terms_and_stems(title)
                reference_intro = self._try_web_augment_answer(
                    f"что такое {title}",
                    q_tokens=q_tokens,
                    q_stems=q_stems,
                    force=True,
                    latency_budget_sec=4.2,
                )
            if reference_intro:
                clean_intro = re.sub(r"\s+", " ", str(reference_intro).strip())
                clean_intro = clean_intro[:1200].rstrip()
                article = "\n".join(
                    [
                        f"# {title}",
                        "",
                        ...
                        "",
                        "## Ключевые аспекты",
                        f"1. Что это такое и как определяется {title}.",
                        f"2. Почему тема «{title}» важна и где применяется.",
                        f"3. Какие примеры, случаи или исторический контекст помогают её понять.",
                        "",
                        "## Короткий вывод",
                        f"Если нужно, я могу сразу продолжить и развернуть статью по теме «{title}» глубже: с примерами, сравнением или историей.",
                    ]
                )
    ...
    def _is_plain_fact_statement(self, message: str) -> bool:
        ...
        if self._is_greeting_intent(text) or self._is_identity_intent(text):
            return False
        if any(re.search(rf"\b{re.escape(marker)}\b", lower) for marker in _DIALOG_FACT_DIRECTIVE_MARKERS):
            return False
        first_clause = re.split(r"[.!;]\s+", text, maxsplit=1)[0].strip()
        if first_clause and first_clause != text and self._is_greeting_intent(first_clause):
            return False
    ...
    def _is_identity_intent(self, text: str) -> bool:
        ...
        if has_user_self_ref:
            return False
        # Для identity route нужна строгая лексика, иначе длинные логические
        # задачи могут случайно попасть в этот path через фонетическую близость.
        exact_tokens = {self._normalize_linguistic_token(tok) for tok in tokens if tok}
        has_you = bool(exact_tokens & _IDENTITY_YOU_WORDS)
        has_name = bool(exact_tokens & _IDENTITY_NAME_WORDS)
    ...
    def _remember_user_fact(self, fact: str) -> None:
        ...
        clean_fact = re.sub(r"^что\s+", "", clean_fact, flags=re.IGNORECASE).strip()
        if len(clean_fact) < 3:
            return
        profile = self._get_user_profile()
    ...
    def _normalize_profile_fact_text(self, fact: str) -> str:
        clean = re.sub(r"\s+", " ", (fact or "").strip())
        clean = re.sub(r"^что\s+", "", clean, flags=re.IGNORECASE).strip()
        return clean

    def _parse_profile_fact_attribute(self, fact: str) -> tuple[str, str] | None:
        clean = self._normalize_profile_fact_text(fact)
        if not clean:
            return None
        match = re.match(
            r"^(?:мой|моя|моё|мое|мои)\s+(.+?)\s*[-:—]\s*(.+)$",
            clean,
            flags=re.IGNORECASE,
        )
        if not match:
    ...
    def _format_profile_fact_statement(self, fact: str) -> str:
        parsed = self._parse_profile_fact_attribute(fact)
        if parsed:
            subject, value = parsed
            return f"ваш {subject} — {value}"
        clean = self._normalize_profile_fact_text(fact)
        if clean.lower().startswith(("я ", "у меня ", "мне ")):
            return clean
        return f"о вас записан такой факт: {clean}"

    def _profile_fact_why_hint(self, fact: str) -> str | None:
        parsed = self._parse_profile_fact_attribute(fact)
        if not parsed:
            return None
        subject, value = parsed
        ...
        if "язык" not in subject_key:
            return f"Вероятно, потому что {value} совпадает с вашими задачами, стилем работы и тем, как вам удобнее думать о задаче."
        language_hints = {
            "rust": "Rust обычно выбирают за контроль над памятью, предсказуемую производительность и надёжность без сборщика мусора.",
            "python": "Python часто подходит за простоту записи, быстрый прототипинг и сильную экосистему библиотек.",
            "typescript": "TypeScript часто выбирают за типизацию, более безопасный рефакторинг и удобную работу в JS-экосистеме.",
            "javascript": "JavaScript обычно подходит, когда важны быстрый старт, браузерная среда и единый язык на клиенте и сервере.",
            "go": "Go часто подходит, когда важны простота, конкурентность и предсказуемая эксплуатация сервисов.",
            "c++": "C++ обычно выбирают за контроль над производительностью, системное программирование и зрелую экосистему native-разработки.",
        }
        for key, hint in language_hints.items():
            if key in value_key:
                return hint
        return f"Вероятно, потому что язык {value} лучше совпадает с вашими задачами, требованиями к скорости разработки и техническими приоритетами."

    def _select_profile_fact_for_query(self, query: str, *, prefer_latest: bool = False) -> str | None:
        profile = self._get_user_profile()
        ...
        for idx, fact in enumerate(facts[-32:]):
            low = fact.lower()
            f_tokens = {t for t in _tokenize(low) if len(t) >= 3 and not _is_stop_word(t)}
            if not f_tokens:
                continue
            ...
            interest_fact = any(k in low for k in ("люблю", "нрав", "интерес", "увлека", "предпоч", "любим"))
            if q_tokens and overlap == 0 and stem_overlap == 0 and not (interest_query and interest_fact):
                continue
            score = overlap * 1.0 + stem_overlap * 0.7 + (0.02 * idx)
            if interest_query and interest_fact:
                score += 1.2
            scored.append((score, fact))
        if not scored:
            if any(k in query_lower for k in ("обо мне", "помнишь", "знаешь")):
                return facts[-1]
            return None
    ...
    def _answer_from_user_facts(self, query: str) -> str | None:
        ...
        query_lower = (query or "").lower()
        why_query = any(k in query_lower for k in ("почему", "зачем"))
        interest_query = any(k in query_lower for k in ("интерес", "люблю", "нрав", "увлека", "предпоч"))
        ...
        if not scored:
            ...
            if any(k in (query or "").lower() for k in ("обо мне", "помнишь", "знаешь")):
                top = facts[-4:]
            elif why_query and any(k in query_lower for k in ("мне", "меня", "мой", "моя", "моё", "мои", "он", "она", "это")):
                top = [facts[-1]]
            else:
                return None
        else:
        ...
        if why_query:
            why_fact = top[0]
            statement = self._format_profile_fact_statement(why_fact).rstrip(" .")
            why_hint = self._profile_fact_why_hint(why_fact)
            if why_hint:
                return f"По моим записям, {statement}. {why_hint}"
        if len(top) == 1:
            statement = self._format_profile_fact_statement(top[0]).rstrip(" .")
            return f"По моим записям, {statement}."
        rendered = [self._format_profile_fact_statement(item) for item in top]
    ...
    def chat(
        ...
        try:
            ...
            # #Фаза: C-Core inference (Prioritized per user request)
            c_formula_answer: dict | None = None
            if self._enable_c_inference and self.c_inference.available and not stage_exceeded("retrieval"):
                ...
                if self._is_valid_c_formula_answer(message, c_formula_answer):
                    ...
                    confidence = float(c_formula_answer.get("confidence", 0.95) or 0.95) # Force high confidence
            ...
            if (
                ...
                if (not projection_query) and any((
                    ...
                    composite_reference_answer = self._try_composite_reference_answer(message)
                    if composite_reference_answer and self._answer_shape_is_valid(message, composite_reference_answer):
                        realtime_direct = composite_reference_answer
                        realtime_method = "web-reference"
                    else:
    ...
    def _is_logic_query(self, message: str) -> bool:
        ...
        if not q:
            return False
        markers = ("логик", "логич", "силлог", "вывод", "следует", "гарантир")
        if any(marker in q for marker in markers):
            return True
        if "все " in q and re.search(r"\b(можно ли сделать вывод|можно ли сказать|следует ли|верно ли)\b", q):
            return True
        if q.count("все ") >= 2 and any(marker in q for marker in ("можно ли", "какой вывод", "следовательно", "значит")):
            return True
        return False
    ...
    def _should_prefer_specialized_pipeline(self, message: str) -> bool:
        ...
        return (
            ...
            or self._is_plain_fact_statement(message)
            or self._is_project_runtime_query(message)
            or self._is_referential_query(message)
        )
    ...
    def _is_c_formula_query(self, message: str) -> bool:
        ...
        if not q:
            return False
        if self._is_logic_query(message) or self._is_math_query(message):
            return False
        patterns = (
            r"\bчто\s+такое\b",
    ...
    def _synthesize_response(
        ...
        composite_reference_answer = self._try_composite_reference_answer(message)
        if composite_reference_answer:
            return composite_reference_answer, 0.9, "web-reference"
        ...
        if self._is_plain_fact_statement(message):
            fact_preview = re.sub(r"\s+", " ", str(message or "").strip()).rstrip(".!?")
            if fact_preview:
                self._remember_user_fact(fact_preview)
                return f"Принял. Зафиксировал в контексте: {fact_preview}.", 0.78, "dialog-fact-ack"
        ...
        if self._is_referential_query(message) or self._is_project_runtime_query(message, q_tokens=q_tokens):
            dynamic_fallback, dynamic_method = self._build_dynamic_no_knowledge_response(
                message=message,
                retrieved_sentences=retrieved_sentences,
                graph_answer=graph_answer,
            ...
            if dynamic_fallback:
                ...
                if dynamic_method == "dialog-fact-ack":
                    return (dynamic_fallback, 0.78, dynamic_method)
                if dynamic_method == "dynamic-fallback":
                    return (dynamic_fallback, 0.22, dynamic_method)
    ...
    def _topic_projection_fallback_response(
        ...
        *,
        ...
        if (
            reference_answer
            and self._answer_shape_is_valid(web_query, reference_answer)
            and not self._response_needs_language_fallback(reference_answer)
        ):
            return {
    ...
    def _build_followup_context_answer(
        ...
        anchor_clean = re.sub(r"\s+", " ", (anchor or "").strip())
        ...
        if mode in {"why", "more", "detail", "simple", "confirm"}:
            profile_anchor = self._select_profile_fact_for_query(anchor_clean or message)
            if profile_anchor:
                statement = self._format_profile_fact_statement(profile_anchor).rstrip(" .")
                if mode == "why":
                    why_hint = self._profile_fact_why_hint(profile_anchor)
                    if why_hint:
                        return f"Если опираться на мои записи, {statement}. {why_hint}"
                if mode == "more":
                    return f"По моим записям, {statement}. Если раскрыть подробнее, это один из ваших зафиксированных предпочтительных ориентиров."
                if mode in {"detail", "simple", "confirm"}:
                    return f"По моим записям, {statement}."
        if anchor_clean and self._is_plain_fact_statement(anchor_clean):
            fact_core = self._strip_memory_ack_wrapper(anchor_clean).rstrip(" .")
            if fact_core:
                if mode == "more":
                    return f"По текущему контексту зафиксирован такой факт: {fact_core}."
                if mode in {"simple", "confirm", "detail"}:
                    return f"{fact_core}."
    ...
    def _build_dynamic_no_knowledge_response(
        ...
        if followup_only and context_window is not None and not self._followup_prefers_fresh_lookup(message, context_window):
        ...
        if effective_weather_query and allow_web_augment and self._enable_web_augment:
            ...
            network_available = external_network_available()
            weather_web = None
            can_try_direct_weather_fetch = network_available or fetch_weather_answer is not _DEFAULT_FETCH_WEATHER_ANSWER
            if weather_hint and can_try_direct_weather_fetch:
                try:
        ...
        if (
            ...
            and self._is_current_reference_query(lookup_message, q_tokens=lookup_q_tokens)
            and allow_web_augment
            and self._enable_web_augment
        ):
            direct_current_answer = self._try_current_reference_answer(lookup_message)
            if direct_current_answer and self._answer_shape_is_valid(lookup_message, direct_current_answer):
                return direct_current_answer, "web-reference"
            current_ref_answer = self._try_web_augment_answer(
                lookup_message,
                q_tokens=lookup_q_tokens,
                q_stems=lookup_q_stems,
                force=True,
                latency_budget_sec=3.8 if fast_mode else 5.6,
            )
            if current_ref_answer and self._answer_shape_is_valid(lookup_message, current_ref_answer):
                return current_ref_answer, "web-augment"
    ...
    def _try_logic_solver_answer(self, query: str) -> str | None:
        ...
        choice_answer = self._solve_choice_implication_puzzle(q)
        if choice_answer:
            return choice_answer

        conditional_answer = self._solve_conditional_property_inference(q)
        ...
        implication_answer = self._solve_transitive_implication(q)
        if implication_answer:
            return implication_answer

        return None

    def _normalize_logic_phrase(self, text: str) -> str:
        clean = re.sub(r"\s+", " ", (text or "").strip().lower().replace("ё", "е"))
        clean = re.sub(r"^[,:\-–—\s]+", "", clean)
        clean = re.sub(r"[.?!,;:]+$", "", clean)
        return clean.strip()

    def _logic_phrases_equivalent(self, left: str, right: str) -> bool:
        left_norm = self._normalize_logic_phrase(left)
        right_norm = self._normalize_logic_phrase(right)
        if not left_norm or not right_norm:
            return False
        if left_norm == right_norm:
            return True
        left_tokens, left_stems = self._extract_terms_and_stems(left_norm, drop_generic=False)
        right_tokens, right_stems = self._extract_terms_and_stems(right_norm, drop_generic=False)
        if left_tokens and right_tokens and left_tokens == right_tokens:
            return True
        shared_stems = left_stems & right_stems
        return bool(shared_stems and len(shared_stems) >= max(1, min(len(left_stems), len(right_stems)) - 1))

    def _logic_singularize_phrase(self, phrase: str) -> str:
        clean = self._normalize_logic_phrase(phrase)
        if not clean:
            return clean
        parts = clean.split()
        if not parts:
            return clean
        first = parts[0]
        if first.endswith("ны") and len(first) >= 4:
            parts[0] = first[:-2] + "ен"
        elif first.endswith("ые") and len(first) >= 4:
            parts[0] = first[:-2] + "ый"
        elif first.endswith("ие") and len(first) >= 4:
            parts[0] = first[:-2] + "ий"
        return " ".join(parts)

    def _logic_render_condition_label(self, phrase: str) -> str:
        clean = self._normalize_logic_phrase(phrase)
        if not clean:
            return clean
        parts = clean.split()
        if len(parts) >= 2:
            first = parts[0]
            last = parts[-1]
            if first.endswith("ыми") and len(first) >= 5:
                parts[0] = first[:-3] + "ые"
            elif first.endswith("ими") and len(first) >= 5:
                parts[0] = first[:-3] + "ие"
            elif first.endswith("ым") and len(first) >= 4:
                parts[0] = first[:-2] + "ый"
            elif first.endswith("им") and len(first) >= 4:
                parts[0] = first[:-2] + "ий"

            if last.endswith("ами") and len(last) >= 5:
                parts[-1] = last[:-3] + "ы"
            elif last.endswith("ями") and len(last) >= 5:
                parts[-1] = last[:-3] + "и"
        return " ".join(parts)

    def _solve_transitive_implication(self, query: str) -> str | None:
        lower = self._normalize_logic_phrase(query)
        if not lower or "все " not in lower:
            return None

        major_match = re.search(
            r"все\s+([а-яёa-z]+)\s+([^,.!?]+?)(?:,\s*а\s+все|\.\s*все|,\s*все|\?|$)",
            lower,
        )
        bridge_match = re.search(
            r"все,\s*кто\s+([^,.!?]+?),\s*([^?.!]+?)(?:,\s*(?:можно ли|следует ли|верно ли)|\?|$)",
            lower,
        )
        if not major_match or not bridge_match:
            return None

        subject_plural = self._normalize_logic_phrase(major_match.group(1))
        bridge_predicate = self._normalize_logic_phrase(major_match.group(2))
        bridge_source = self._normalize_logic_phrase(bridge_match.group(1))
        conclusion_predicate = self._normalize_logic_phrase(bridge_match.group(2))
        if not subject_plural or not bridge_predicate or not bridge_source or not conclusion_predicate:
            return None
        if not self._logic_phrases_equivalent(bridge_predicate, bridge_source):
            return None
        ...
        singular_map = {
            ...
            "млекопитающие": "млекопитающее",
        }
        subject_singular = singular_map.get(subject_plural, subject_plural[:-1] if len(subject_plural) > 4 else subject_plural)
        subject_display = subject_plural[:1].upper() + subject_plural[1:]
        singular_predicate = self._logic_singularize_phrase(conclusion_predicate)
        return (
            "Логический разбор:\n"
            f"1) Все {subject_plural} {bridge_predicate}.\n"
            f"2) Все, кто {bridge_source}, {conclusion_predicate}.\n"
            f"Вывод: да, {subject_display} {conclusion_predicate}, а значит отдельный {subject_singular} тоже {singular_predicate}."
        )

    def _solve_conditional_property_inference(self, query: str) -> str | None:
        lower = self._normalize_logic_phrase(query)
        if not lower.startswith("если все "):
            return None
        universal_match = re.search(
            r"если\s+все\s+([а-яёa-z0-9_-]+)\s+с\s+([^,.!?]+?)\s+([^,.!?]+?)(?:,\s*а|\.\s*а)",
            lower,
        )
        entity_match = re.search(
            r"(?:,\s*а|\.\s*а)\s*([а-яёa-z0-9_-]+)\s+([^,.!?]+?)(?:,\s*можно ли|,\s*следует ли|\?|$)",
            lower,
        )
        if not universal_match or not entity_match:
            return None

        class_name = self._normalize_logic_phrase(universal_match.group(1))
        condition = self._normalize_logic_phrase(universal_match.group(2))
        predicate = self._normalize_logic_phrase(universal_match.group(3))
        subject = entity_match.group(1).strip()
        relation_text = self._normalize_logic_phrase(entity_match.group(2))
        if not class_name or not condition or not predicate or not subject or not relation_text:
            return None

        weak_markers = ("стремится к", "пытается", "хочет", "идет к", "идёт к", "движется к", "приближается к")
        strong_patterns = (
            r"^(?:имеет|обладает|характеризуется|отличается)\s+(.+)$",
            r"^с\s+(.+)$",
        )

        relation_property = relation_text
        relation_strength = "unknown"
        for pattern in strong_patterns:
            match = re.match(pattern, relation_text)
            if match:
                relation_property = self._normalize_logic_phrase(match.group(1))
                relation_strength = "strong"
                break
        if relation_strength == "unknown":
            for marker in weak_markers:
                if relation_text.startswith(marker):
                    relation_property = self._normalize_logic_phrase(relation_text[len(marker):].strip())
        ...
        pred_tokens = predicate.split()
        rel_tokens = relation_property.split()
        rendered_condition = condition
        if pred_tokens and len(pred_tokens) >= 2:
            compound_condition = self._normalize_logic_phrase(" ".join([condition, pred_tokens[0]]))
            if self._logic_phrases_equivalent(compound_condition, relation_property):
                rendered_condition = compound_condition
                predicate = " ".join(pred_tokens[1:])
                pred_tokens = predicate.split()
        if rel_tokens and len(rel_tokens) > len(rendered_condition.split()) and self._logic_phrases_equivalent(rendered_condition, relation_property):
            rendered_condition = relation_property
        if pred_tokens and rel_tokens:
            pred_head_stem = _stem_ru(pred_tokens[0])
            rel_tail_stem = _stem_ru(rel_tokens[-1])
            if pred_head_stem and rel_tail_stem and pred_head_stem == rel_tail_stem and len(pred_tokens) >= 2:
                predicate = " ".join(pred_tokens[1:])

        rendered_condition_label = self._logic_render_condition_label(rendered_condition)
        subject_display = subject[:1].upper() + subject[1:]
        if self._logic_phrases_equivalent(condition, relation_property):
            if relation_strength == "strong":
                return (
                    "Логический разбор:\n"
                    f"1) Все {class_name}, у которых есть «{rendered_condition_label}», {predicate}.\n"
                    f"2) {subject_display} действительно имеет «{rendered_condition_label}».\n"
                    f"Вывод: да, {subject_display} {predicate}."
                )
            return (
                "Логический разбор:\n"
                f"1) Все {class_name}, у которых есть «{rendered_condition_label}», {predicate}.\n"
                f"2) Про {subject_display} сказано только то, что он {relation_text}, а это слабее, чем уже иметь «{rendered_condition_label}».\n"
                f"Вывод: строгий вывод, что {subject_display} {predicate}, пока сделать нельзя; для него нужна более сильная посылка."
            )
        return None

    def _solve_choice_implication_puzzle(self, query: str) -> str | None:
        text = re.sub(r"\s+", " ", str(query or "").strip())
        if not text:
            return None
        options = re.findall(r"\b(?:либо|или)\s+([A-ZА-Я])\b", text)
        ...
        implications: list[tuple[str, bool, str, bool]] = []
        for match in re.finditer(
            r"если\s+(не\s+)?([A-ZА-Я])\s*,?\s*то\s+(не\s+)?([A-ZА-Я])",
            text,
            flags=re.IGNORECASE,
        ):
            implications.append((
        ...
        valid_assignments: list[dict[str, bool]] = []
        for chosen in unique_options:
            assignment = {option: option == chosen for option in unique_options}
            ok = True
            for src, src_neg, dst, dst_neg in implications:
                src_value = assignment.get(src, False)
                antecedent = (not src_value) if src_neg else src_value
                if not antecedent:
                    continue
                dst_value = assignment.get(dst, False)
                consequent = (not dst_value) if dst_neg else dst_value
                if not consequent:
                    ok = False
                    break
            if ok:
        ...
        if not valid_assignments:
            return "Логический разбор: условия противоречат друг другу, поэтому допустимого выбора здесь нет."

        valid_choices = [option for option in unique_options if any(assignment.get(option, False) for assignment in valid_assignments)]
        excluded = [option for option in unique_options if option not in valid_choices]
        if len(valid_choices) == 1:
            choice = valid_choices[0]
            excluded_text = f" Варианты {', '.join(excluded)} исключаются." if excluded else ""
            return (
                "Логический разбор:\n"
                f"1) По условию нужно выбрать ровно один вариант: {', '.join(unique_options)}.\n"
                f"2) Импликации отсекают все варианты, кроме {choice}.{excluded_text}\n"
                f"Вывод: нужно выбрать {choice}."
            )
        ...
        return (
            "Логический разбор:\n"
            f"1) По условию нужно выбрать ровно один вариант: {', '.join(unique_options)}.\n"
            f"2) Проверка импликаций показывает, что совместимы варианты {', '.join(valid_choices)}.{excluded_text}\n"
            f"Вывод: однозначно выбрать один вариант нельзя. Если {valid_choices[0]} отвергается, тогда остаётся {valid_choices[1]}."
        )
    ...
    def _cognitive_enrichment(self, message: str) -> dict:
        ...
        try:
            ...
            # Абстрактное мышление — принудительная глубина 3
            abstract_result = cog.abstract(message, depth=3)

            # Каузальная цепочка — всегда пытаемся найти связи
            causal_chain: list[tuple[str, float]] = []
            why_result = cog.why(message, max_chain=5)
    ...
    def _build_profile_memory_read_response(self, message: str) -> tuple[str | None, str]:
        ...
        if any(token in stripped for token in ("как меня зовут", "кто я")):
            profile = self._get_user_profile()
            name = str(profile.get("name", "") or "").strip()
            if name:
                return f"Вас зовут {name}.", "profile-memory"
            return (
                "Я пока не помню ваше имя. Напишите, например: `Меня зовут Алексей`.",
                "profile-memory-empty",
            )

        if any(token in stripped for token in ("что ты знаешь обо мне", "что ты помнишь обо мне")):
        ...
        referential_personal_query = (
            not is_memory_command
            and self._is_referential_query(original)
            and any(token in stripped for token in ("мне", "меня", "мой", "моя", "моё", "мои", "обо мне", "про меня"))
        )
        is_personal_query = (any(token in stripped for token in personal_query_tokens) or referential_personal_query) and not is_memory_command
    ...
    def _build_system_read_response(self, message: str) -> tuple[str | None, str]:
        ...
        first_clause = re.split(r"[.!;]\s+", original, maxsplit=1)[0].strip()
        ...
        if first_clause and first_clause != original and self._is_greeting_intent(first_clause):
            profile = self._get_user_profile()
            user_name = str(profile.get("name", "") or "").strip()
            name_part = f", {user_name}" if user_name else ""
            return (
                f"Привет{name_part}. Я {self._assistant_name}. "
                "Готов ответить коротко и по делу. Задайте вопрос или продолжите следующим сообщением."
            ), "greeting"
    ...
    def _handle_special_commands(self, message: str, lower: str, context_window: ContextWindow | None = None) -> dict | None:
        ...
        if stripped in {"как меня зовут", "кто я"}:
            profile = self._get_user_profile()
            user_name = str(profile.get("name", "") or "").strip()
            if user_name:
                return {
                    "response": f"Вас зовут {user_name}.",
                    "confidence": 0.99,
            ...
            return {
                "response": "Я пока не помню ваше имя. Напишите, например: `Меня зовут Алексей`.",
                "confidence": 0.9,
                "sources": ["profile-memory"],
        ...
        if content_gen_match:
            ...
            return self._generate_article(topic, context_window, request_message=original)
        if self._is_longform_writing_intent(original):
            topic = self._extract_longform_topic(original)
            if topic:
                return self._generate_article(topic, context_window, request_message=original)
        ...
        if auto_fact:
            ...
            fact_statement = self._format_profile_fact_statement(auto_fact).rstrip(" .")
            return {
                "response": f"Хорошо, запомнил: {fact_statement}.",
                "confidence": 0.98,
                "sources": ["profile-memory"],
        ...
        if stripped.startswith(("запомни", "remember")):
            ...
            if len(fact_text) < 3:
                ...
                fact_text = re.sub(r"^что\s+", "", fact_text, flags=re.IGNORECASE).strip()
                self._remember_user_fact(fact_text)
                fact_statement = self._format_profile_fact_statement(fact_text).rstrip(" .")
                resp = f"Хорошо, запомнил: {fact_statement}."
            return {
                "response": resp,
                "confidence": 1.0,
                "sources": ["profile-memory"],
