...
_CURRENCY_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "BTC": ("btc", "bitcoin", "биткоин", "биткойн", "биток"),
    "ETH": ("eth", "ethereum", "эфир", "эфириум"),
    "USDT": ("usdt", "tether", "тезер"),
}
_CRYPTO_PRICE_IDS: dict[str, str] = {
    "BTC": "bitcoin",
    "ETH": "ethereum",
    "USDT": "tether",
}
...
def looks_like_currency_query(query: str) -> bool:
    ...
    if len(codes) >= 2:
        return True
    if len(codes) == 1 and any(marker in low for marker in ("цена", "стоит", "price", "стоимость")):
        return True
...
def _parse_currency_request(query: str) -> tuple[float, str, str] | None:
    ...
    if len(codes) >= 2:
        ...
        if any(marker in low for marker in ("к руб", "в руб", "рубл", "to rub")) and from_code != "RUB":
            ...
            to_code = "EUR"
        elif from_code in _CRYPTO_PRICE_IDS:
            to_code = "USD"
        elif from_code == "RUB":
            to_code = "USD"
        else:
...
def fetch_exchange_rate_answer(query: str, *, timeout: float = 5.0) -> str | None:
    ...
    if from_code in _CRYPTO_PRICE_IDS or to_code in _CRYPTO_PRICE_IDS:
        return _fetch_crypto_price_answer(amount, from_code, to_code, timeout=timeout)
    try:
        resp = _requests_get(
...
def _fetch_crypto_price_answer(amount: float, from_code: str, to_code: str, *, timeout: float = 5.0) -> str | None:
    fiat_targets = {"USD", "EUR", "RUB"}
    if from_code in _CRYPTO_PRICE_IDS and to_code in fiat_targets:
        crypto_code = from_code
        fiat_code = to_code
        inverse = False
    elif to_code in _CRYPTO_PRICE_IDS and from_code in fiat_targets:
        crypto_code = to_code
        fiat_code = from_code
        inverse = True
    else:
        return None
    vs_currency = fiat_code.lower()
    try:
        resp = _requests_get(
            "https://api.coingecko.com/api/v3/simple/price",
            params={"ids": _CRYPTO_PRICE_IDS[crypto_code], "vs_currencies": vs_currency},
            headers=_HEADERS,
            timeout=max(2.0, timeout),
        )
        resp.raise_for_status()
        ...
        return None
    try:
        rate = float(data[_CRYPTO_PRICE_IDS[crypto_code]][vs_currency])
    except Exception:
        return None
    if rate <= 0:
        return None
    ...
    if inverse:
        converted = amount / rate
        unit_answer = f"По цене на {updated}, 1 {crypto_code} ≈ {_format_rate_value(rate)} {fiat_code}."
        if abs(amount - 1.0) < 1e-9:
            return unit_answer
        return f"{unit_answer} {_format_rate_value(amount)} {from_code} ≈ {_format_rate_value(converted)} {to_code}."
    converted = amount * rate
    if abs(amount - 1.0) < 1e-9:
        return f"По цене на {updated}, 1 {from_code} ≈ {_format_rate_value(rate)} {to_code}."
    return (
        f"По цене на {updated}, 1 {from_code} ≈ {_format_rate_value(rate)} {to_code}. "
        f"{_format_rate_value(amount)} {from_code} ≈ {_format_rate_value(converted)} {to_code}."
    )
