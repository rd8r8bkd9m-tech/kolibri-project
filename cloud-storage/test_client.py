...
def upload_file(token, file_path):
    ...
    with open(file_path, "rb") as f:
        files = {"file": f}
        response = requests.post(
            f"{API_URL}/storage/upload", headers=headers, files=files
        )
...
def download_file(token, file_id, output_path):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{API_URL}/storage/download/{file_id}", headers=headers)
...
def delete_file(token, file_id):
    ...
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.delete(f"{API_URL}/storage/files/{file_id}", headers=headers)
