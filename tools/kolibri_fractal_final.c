/*
 * KOLIBRI FRACTAL v40 - 4 уровня + смешанный контекст
 * Берём лучшее из v37 + улучшаем контекст для bits0
 */
...
/* Binary model с run-length контекстом */
typedef struct { uint16_t prob[64]; } BinModelRL;
static void bmrl_init(BinModelRL *m) { 
    for (int i = 0; i < 64; i++) m->prob[i] = 2048; 
}
static void bmrl_update(BinModelRL *m, int ctx, int bit) {
    ctx &= 63;
    if (bit) m->prob[ctx] += (uint16_t)((4096 - m->prob[ctx]) >> 5);
    else m->prob[ctx] -= m->prob[ctx] >> 5;
}
static void rc_enc_bitrl(RC *rc, BinModelRL *m, int ctx, int bit) {
    ctx &= 63;
    uint32_t bound = (rc->range >> 12) * m->prob[ctx];
    if (bit) rc->range = bound;
    else { rc->low += bound; rc->range -= bound; }
    rc_enc_norm(rc);
    bmrl_update(m, ctx, bit);
}
static int rc_dec_bitrl(RC *rc, BinModelRL *m, int ctx) {
    ctx &= 63;
    uint32_t bound = (rc->range >> 12) * m->prob[ctx];
...
static int compress_file(const char *in_path, const char *out_path) {
    ...
    /* B0: Run-length контекст (bits + run length) */
    uint8_t *bits0 = malloc(buf);
    BinModelRL bm0; bmrl_init(&bm0);
    rc_enc_init(&rc, bits0, buf);
    int run = 0;  /* длина текущего run нулей */
    int lastb = 0;
    for (size_t i = 0; i < in_size; i++) {
        int b = (mtf[i] != 0);
        /* Контекст: prev bits (4) + run length class (2) */
        int rl_class = (run == 0) ? 0 : (run < 4) ? 1 : (run < 16) ? 2 : 3;
        int ctx = ((lastb & 15) << 2) | rl_class;
        rc_enc_bitrl(&rc, &bm0, ctx, b);
        lastb = (lastb << 1) | b;
        if (!b) run++; else run = 0;
    }
    rc_enc_flush(&rc);
    ...
    size_t sf = sb0 + sb1 + sb2 + sb3 + sv1 + sv2 + sv3 + sv4 + 44;
    
    printf("=== v40 ===\n");
    printf("B0(rl): %zu, B1: %zu, B2: %zu, B3: %zu\n", sb0, sb1, sb2, sb3);
    ...
    write32(out, &pos, crc);
    write32(out, &pos, (uint32_t)sb0);
    write32(out, &pos, (uint32_t)sb1);
    write32(out, &pos, (uint32_t)sb2);
    write32(out, &pos, (uint32_t)sb3);
    write32(out, &pos, (uint32_t)sv1);
    write32(out, &pos, (uint32_t)sv2);
    write32(out, &pos, (uint32_t)sv3);
...
static int decompress_file(const char *in_path, const char *out_path) {
    ...
    if (magic != MAGIC) { free(in_data); return 1; }
    
    uint32_t orig = read32(in_data, &pos);
    uint32_t bwt_idx = read32(in_data, &pos);
    uint32_t stored_crc = read32(in_data, &pos);
    uint32_t sb0 = read32(in_data, &pos);
    uint32_t sb1 = read32(in_data, &pos);
    uint32_t sb2 = read32(in_data, &pos);
    uint32_t sb3 = read32(in_data, &pos);
    uint32_t sv1 = read32(in_data, &pos);
    uint32_t sv2 = read32(in_data, &pos);
    uint32_t sv3 = read32(in_data, &pos);
    ...
    for (size_t i = 0; i < orig; i++) {
        int rl_class = (run == 0) ? 0 : (run < 4) ? 1 : (run < 16) ? 2 : 3;
        int ctx = ((lastb & 15) << 2) | rl_class;
        db0[i] = (uint8_t)rc_dec_bitrl(&rc, &bm0, ctx);
        lastb = (lastb << 1) | db0[i];
        if (!db0[i]) run++; else run = 0;
    }
    pos += sb0;
    ...
    for (size_t i = 0; i < nz; i++) {
        db1[i] = (uint8_t)rc_dec_bit4(&rc, &bm1, ctx);
        ctx = (ctx << 1) | db1[i];
    }
    pos += sb1;
    ...
    for (size_t i = 0; i < n4p; i++) {
        db2[i] = (uint8_t)rc_dec_bit4(&rc, &bm2, ctx);
        ctx = (ctx << 1) | db2[i];
    }
    pos += sb2;
    ...
    for (size_t i = 0; i < n10p; i++) {
        db3[i] = (uint8_t)rc_dec_bit4(&rc, &bm3, ctx);
        ctx = (ctx << 1) | db3[i];
    }
    pos += sb3;
    ...
    for (size_t i = 0; i < c13; i++) {
        int c = p2 * 3 + p1;
        dv1[i] = (uint8_t)rc_dec_symN2(&rc, &m3, c);
        p2 = p1; p1 = dv1[i];
    }
    pos += sv1;
    ...
    for (size_t i = 0; i < c49; i++) {
        int c = p2 * 6 + p1;
        if (c >= 36) c = 35;
        dv2[i] = (uint8_t)rc_dec_symN2(&rc, &m6, c);
        p2 = p1; p1 = dv2[i];
    }
    pos += sv2;
    ...
    for (size_t i = 0; i < c1031; i++) {
        int c = (p2 % 6) * 6 + (p1 % 6);
        dv3[i] = (uint8_t)rc_dec_symN2(&rc, &m22, c);
        p2 = p1; p1 = dv3[i];
    }
    pos += sv3;
