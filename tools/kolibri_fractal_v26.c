 ...
 * Исправленная версия без buffer overflow
 */
...
/* Order-1 модель для бинарного алфавита */
typedef struct {
    uint16_t freq0[2], freq1[2], total[2];
} Model2;

static void m2_init(Model2 *m) {
    m->freq0[0] = m->freq0[1] = 1;
    m->freq1[0] = m->freq1[1] = 1;
    m->total[0] = m->total[1] = 2;
}

static void m2_update(Model2 *m, uint8_t ctx, uint8_t bit) {
    if (bit) m->freq1[ctx] += 16;
    else m->freq0[ctx] += 16;
    m->total[ctx] += 16;
    if (m->total[ctx] > 0x3FFF) {
        m->freq0[ctx] = (m->freq0[ctx] >> 1) | 1;
        m->freq1[ctx] = (m->freq1[ctx] >> 1) | 1;
        m->total[ctx] = m->freq0[ctx] + m->freq1[ctx];
    }
}
...
static void rc_enc_bit(RC *rc, Model2 *m, uint8_t ctx, uint8_t bit) {
    rc->range /= m->total[ctx];
    if (bit) {
        rc->low += m->freq0[ctx] * rc->range;
        rc->range *= m->freq1[ctx];
    } else {
        rc->range *= m->freq0[ctx];
    }
    rc_enc_norm(rc);
    m2_update(m, ctx, bit);
}
...
static uint8_t rc_dec_bit(RC *rc, Model2 *m, uint8_t ctx) {
    rc->range /= m->total[ctx];
    uint32_t threshold = m->freq0[ctx] * rc->range;
...
static int compress_file(const char *in_path, const char *out_path) {
    ...
    printf("Ненули: %zu (%.1f%%)\n", nz_count, 100.0*nz_count/in_size);
    
    /* Выделяем буферы с запасом */
    size_t buf_size = in_size + 1024;
    
    /* Метод 1: Order-1 */
    uint8_t *comp1 = malloc(buf_size);
    RC rc;
    Model256 *m256 = malloc(sizeof(Model256));
    m256_init(m256);
    rc_enc_init(&rc, comp1, buf_size);
    uint8_t ctx = 0;
    for (size_t i = 0; i < in_size; i++) {
    ...
    /* Метод 2: Фрактальный (биты + значения) */
    uint8_t *nonzero = malloc(nz_count + 1);
    size_t ni = 0;
    for (size_t i = 0; i < in_size; i++) {
        if (mtf_out[i] != 0) nonzero[ni++] = mtf_out[i];
    }
    
    /* Биты */
    uint8_t *comp_bits = malloc(buf_size);
    Model2 m2;
    m2_init(&m2);
    rc_enc_init(&rc, comp_bits, buf_size);
    uint8_t bctx = 0;
    for (size_t i = 0; i < in_size; i++) {
        uint8_t bit = (mtf_out[i] != 0) ? 1 : 0;
        rc_enc_bit(&rc, &m2, bctx, bit);
        bctx = bit;
    }
    rc_enc_flush(&rc);
    ...
    /* Значения */
    uint8_t *comp_vals = malloc(buf_size);
    m256_init(m256);
    rc_enc_init(&rc, comp_vals, buf_size);
    ctx = 0;
    ...
    size_t size2 = bits_size + vals_size + 4;  /* +4 для размера битов */
    printf("Метод 2 (Фрактальный): биты=%zu + значения=%zu = %zu байт (%.2fx)\n",
           bits_size, vals_size, size2, (double)in_size/size2);
    
    /* Выбираем лучший */
    int use_fractal = (size2 < size1);
    printf("Выбран: %s\n", use_fractal ? "Фрактальный" : "Order-1");
    
    /* Заголовок: 20 байт */
    size_t hdr = 20;
    size_t data_size = use_fractal ? size2 : size1;
    uint8_t *out = malloc(hdr + data_size);
    ...
    if (use_fractal) {
        ...
        memcpy(out + 20, comp1, size1);
    }
    
    size_t comp_size = hdr + data_size;
    ...
    free(m256); free(comp1); free(comp_bits); free(comp_vals); free(nonzero);
...
static int decompress_file(const char *in_path, const char *out_path) {
    ...
    if (method == 1) {
        ...
        uint8_t *vals = malloc(nz_count + 1);
        uint8_t ctx = 0;
        for (size_t i = 0; i < nz_count; i++) {
