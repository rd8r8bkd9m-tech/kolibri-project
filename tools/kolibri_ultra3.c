/*
 * KOLIBRI ULTRA v31 - Исправленный 4-уровневый фрактальный кодер
 * С правильным BWT decode из v27
 */
...
static size_t compress_ultra(const uint8_t *in, size_t len, uint8_t *out) {
    ...
    uint8_t *bits1 = malloc(non_zero + 1);      
    uint8_t *bits2 = malloc(lvl2_cnt + 1);      
    uint8_t *vals1 = malloc(cnt[1] + 1);        
    uint8_t *vals2 = malloc(cnt[2] + 1);        
    uint8_t *vals3 = malloc(cnt[3] + 1);        
...
static size_t decompress_ultra(const uint8_t *in, size_t in_len, uint8_t *out) {
    ...
    if (ptr[0] != 'K' || ptr[1] != 'U' || ptr[2] != 'L' || ptr[3] != '3') {
    ...
    uint8_t *bits1 = malloc(non_zero + 1);
    uint8_t *bits2 = malloc(lvl2_cnt + 1);
    uint8_t *vals1 = malloc(cnt1 + 1);
    uint8_t *vals2 = malloc(cnt2 + 1);
    uint8_t *vals3 = malloc(cnt3 + 1);
