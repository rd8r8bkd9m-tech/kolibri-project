/*
 * KOLIBRI FRACTAL v37 - 4 уровня + Order-3 bits + оптимизация V3
 * 0 / 1-3 / 4-9 / 10-31 / 32+
 */
...
/* Binary model Order-N */
typedef struct { uint16_t prob[16]; int order; } BinModelN;
static void bmn_init(BinModelN *m, int order) { 
    m->order = order;
    for (int i = 0; i < (1 << order); i++) m->prob[i] = 2048; 
}
static void bmn_update(BinModelN *m, int ctx, int bit) {
    if (bit) m->prob[ctx] += (4096 - m->prob[ctx]) >> 5;
    else m->prob[ctx] -= m->prob[ctx] >> 5;
}
static void rc_enc_bitn(RC *rc, BinModelN *m, int ctx, int bit) {
    int mask = (1 << m->order) - 1;
    ctx &= mask;
    uint32_t bound = (rc->range >> 12) * m->prob[ctx];
    if (bit) rc->range = bound;
    else { rc->low += bound; rc->range -= bound; }
    rc_enc_norm(rc);
    bmn_update(m, ctx, bit);
}
static int rc_dec_bitn(RC *rc, BinModelN *m, int ctx) {
    int mask = (1 << m->order) - 1;
    ctx &= mask;
    uint32_t bound = (rc->range >> 12) * m->prob[ctx];
...
static int compress_file(const char *in_path, const char *out_path) {
    ...
    printf("Распределение: 0=%zu, 1-3=%zu, 4-9=%zu, 10-31=%zu, 32+=%zu\n", 
           cnt[0], cnt[1], cnt[2], cnt[3], cnt[4]);
    
    size_t buf = in_size + 1024;
    RC rc;
    
    /* B0: 0 vs non-0 (Order-4) */
    uint8_t *bits0 = malloc(buf);
    BinModelN bm0; bmn_init(&bm0, 4);
    rc_enc_init(&rc, bits0, buf);
    int ctx = 0;
    for (size_t i = 0; i < in_size; i++) {
        int b = (mtf[i] != 0);
        rc_enc_bitn(&rc, &bm0, ctx, b);
        ctx = (ctx << 1) | b;
    }
    rc_enc_flush(&rc);
    ...
    BinModelN bm1; bmn_init(&bm1, 4);
    rc_enc_init(&rc, bits1, buf);
    ctx = 0;
    for (size_t i = 0; i < in_size; i++) {
        if (mtf[i] > 0) {
            int b = (mtf[i] > 3);
            rc_enc_bitn(&rc, &bm1, ctx, b);
            ctx = (ctx << 1) | b;
        }
    }
    rc_enc_flush(&rc);
    ...
    /* B2: 4-9 vs 10+ (Order-4) */
    uint8_t *bits2 = malloc(buf);
    BinModelN bm2; bmn_init(&bm2, 4);
    rc_enc_init(&rc, bits2, buf);
    ctx = 0;
    for (size_t i = 0; i < in_size; i++) {
        if (mtf[i] > 3) {
            int b = (mtf[i] > 9);
            rc_enc_bitn(&rc, &bm2, ctx, b);
            ctx = (ctx << 1) | b;
        }
    }
    rc_enc_flush(&rc);
    ...
    /* B3: 10-31 vs 32+ (Order-4) */
    uint8_t *bits3 = malloc(buf);
    BinModelN bm3; bmn_init(&bm3, 4);
    rc_enc_init(&rc, bits3, buf);
    ctx = 0;
    for (size_t i = 0; i < in_size; i++) {
        if (mtf[i] > 9) {
            int b = (mtf[i] > 31);
            rc_enc_bitn(&rc, &bm3, ctx, b);
            ctx = (ctx << 1) | b;
        }
    }
    rc_enc_flush(&rc);
