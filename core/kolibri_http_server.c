...
#define STATIC_DIR_PATH "web/dist"
...
/* Mutex moved to KwmContext */
static pthread_mutex_t g_wm_mutex = PTHREAD_MUTEX_INITIALIZER; /* Protects World Model from concurrent access */
...
static void *bg_learn_loop(void *arg) {
    ...
    while (!g_bg_learn_stop) {
        ...
        /* Train 1 tick per cycle — protected by mutex */
        float loss = kal_train_tick(g_auto_learn);
        g_bg_learn_ticks++;
        g_bg_learn_current_loss = loss;
...
/**
 * generate_neural_response - Генерация ответа через World Model (Transformer)
 *
 * Это ЯДРО настоящего ИИ Колибри:
 * 1. Создаёт prompt из контекста + вопроса
 * 2. Использует Transformer для генерации токенов
 * 3. Возвращает сгенерированный ответ с confidence на основе perplexity
 *
 * @param prompt - входной контекст/вопрос
 * @param output - буфер для сгенерированного ответа
 * @param output_size - размер буфера
 * @param confidence_out - уверенность модели (0.0-1.0)
 * @return 0 при успехе, -1 при ошибке
 */
static int generate_neural_response(const char *prompt, char *output, size_t output_size, float *confidence_out) {
    if (!g_world_model_ready || !g_world_model) {
        return -1; // World Model не готова
    }

    if (!prompt || !output || !confidence_out) {
        return -1;
    }

    // Обучаем World Model на prompt (контекст)
    ...
    size_t gen_len = kwm_generate(g_world_model, generated, sizeof(generated) - 1, 0.8);

    if (gen_len == 0) {
        return -1; // Генерация не удалась
    }

    // Копируем сгенерированный текст
    generated[gen_len] = '\0';
    snprintf(output, output_size, "%s", generated);

    // Получаем статистику для расчёта confidence
    KwmStats stats;
    kwm_get_stats(g_world_model, &stats);

    // Рассчитываем confidence на основе perplexity и loss
    // Чем ниже loss, тем выше уверенность
    double avg_loss = stats.avg_loss > 0 ? stats.avg_loss : 4.0; // fallback
    float confidence = (float)(1.0 / (1.0 + avg_loss / 4.0));    // Нормализация в [0, 1]

    // Clamp confidence в разумных пределах
    if (confidence < 0.3)
        confidence = 0.3;
    if (confidence > 0.95)
        confidence = 0.95;
