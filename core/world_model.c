...
KwmContext *kwm_create(uint64_t seed) {
    ...
    ctx->auto_learn = 0; /* disabled to prevent concurrent kat_train_step_fast */

    /* Mutex для защиты от concurrent access */
...
void kwm_destroy(KwmContext *ctx) {
    if (!ctx)
        return;
    if (ctx->workspace)
        kat_workspace_destroy((KatWorkspace *)ctx->workspace);
    if (ctx->backbone)
        kat_model_destroy((KatModel *)ctx->backbone);
...
float kwm_observe(KwmContext *ctx, uint8_t byte, KwmPrediction *pred) {
    ...
    if (ctx->context_len > 0) {
        ...
        if (seq_len > KWM_CONTEXT_SIZE)
            seq_len = KWM_CONTEXT_SIZE;

        int rc = kat_forward((KatModel *)ctx->backbone, (KatWorkspace *)ctx->workspace, ctx->context, seq_len);
        ...
        if (rc == 0) {
            ...
            if (ctx->auto_learn && (ctx->tick & 3) == 0) {
                float loss = kat_train_step_fast((KatModel *)ctx->backbone, (KatWorkspace *)ctx->workspace,
                                                 ctx->context, seq_len, byte, ctx->learning_rate);
    ...
    if (surprise > ctx->surprise_threshold && ctx->concept_count < KWM_MAX_CONCEPTS && ctx->context_len >= 4) {
        ...
        /* Извлекаем эмбеддинг */
        kat_extract_embedding((KatWorkspace *)ctx->workspace, concept->embedding);
...
int kwm_predict(KwmContext *ctx, KwmPrediction *pred) {
    ...
    /* Forward pass */
    int rc = kat_forward((KatModel *)ctx->backbone, (KatWorkspace *)ctx->workspace, ctx->context, ctx->context_len);
...
size_t kwm_generate(KwmContext *ctx, uint8_t *output, size_t max_len, float temperature) {
    if (!ctx || !output || max_len == 0)
        return 0;
    ...
    for (size_t i = 0; i < max_len; i++) {
        ...
        uint8_t next = kat_sample((KatModel *)ctx->backbone, (KatWorkspace *)ctx->workspace, temperature);
...
int kwm_embed_text(KwmContext *ctx, const char *text, size_t len, float *out) {
    if (!ctx || !text || len == 0 || !out)
        return -1;

    pthread_mutex_lock(&ctx->mutex);
    /* Ограничиваем длину контекстным окном */
    if (len > KWM_CONTEXT_SIZE)
        len = KWM_CONTEXT_SIZE;

    /* Forward pass */
    int rc = kat_forward((KatModel *)ctx->backbone, (KatWorkspace *)ctx->workspace, (const uint8_t *)text, len);
...
float kwm_similarity(KwmContext *ctx, const char *a, const char *b) {
    ...
    if (kwm_embed_text(ctx, a, strlen(a), emb_a) != 0)
        return 0.0f;
    if (kwm_embed_text(ctx, b, strlen(b), emb_b) != 0)
        return 0.0f;

    return kat_cosine_similarity(emb_a, emb_b, KWM_CONCEPT_DIM);
}

size_t kwm_extract_concepts(KwmContext *ctx, const char *text, size_t len, KwmConcept *concepts, size_t max_concepts) {
    if (!ctx || !text || len == 0 || !concepts || max_concepts == 0)
        return 0;
    ...
    for (size_t pos = 0; pos + window <= len && found < max_concepts; pos += window / 2) {
        float emb[KWM_CONCEPT_DIM];

        /* Эмбеддинг окна */
        if (kwm_embed_text(ctx, text + pos, (pos + window > len) ? len - pos : window, emb) != 0)
            continue;

        /* Проверяем, не повторяет ли существующий концепт */
        int is_new = 1;
        for (size_t i = 0; i < found; i++) {
            float sim = kat_cosine_similarity(emb, concepts[i].embedding, KWM_CONCEPT_DIM);
...
float kwm_learn_step(KwmContext *ctx) {
    ...
    return kat_train_step_fast((KatModel *)ctx->backbone, (KatWorkspace *)ctx->workspace, ctx->context, input_len,
...
size_t kwm_serialize(const KwmContext *ctx, uint8_t *buf, size_t buf_size) {
    ...
    size_t backbone_size = kat_serialize((const KatModel *)ctx->backbone, NULL, 0);
    size_t total = sizeof(uint64_t)     /* tick */
                   ...
                   + ctx->concept_count * sizeof(KwmConcept) + backbone_size;
    ...
    /* Состояние */
    memcpy(p, &ctx->tick, sizeof(uint64_t));
    p += sizeof(uint64_t);
    memcpy(p, &ctx->context_len, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(p, &ctx->context_pos, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(p, &ctx->history_len, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(p, &ctx->history_pos, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(p, ctx->context, KWM_CONTEXT_SIZE);
    p += KWM_CONTEXT_SIZE;
    memcpy(p, ctx->history, KWM_HISTORY_SIZE);
    p += KWM_HISTORY_SIZE;
    memcpy(p, &ctx->stats, sizeof(KwmStats));
    p += sizeof(KwmStats);
    memcpy(p, &ctx->learning_rate, sizeof(float));
    p += sizeof(float);
    memcpy(p, &ctx->surprise_threshold, sizeof(float));
    p += sizeof(float);
    memcpy(p, &ctx->auto_learn, sizeof(int));
    p += sizeof(int);
    memcpy(p, &ctx->concept_count, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(p, ctx->concepts, ctx->concept_count * sizeof(KwmConcept));
    p += ctx->concept_count * sizeof(KwmConcept);

    /* Бэкбон */
    kat_serialize((const KatModel *)ctx->backbone, p, buf_size - (size_t)(p - buf));

    return total;
}

int kwm_deserialize(KwmContext *ctx, const uint8_t *buf, size_t buf_size) {
    if (!ctx || !buf || buf_size < sizeof(uint64_t))
        return -1;

    const uint8_t *p = buf;

    /* Состояние */
    memcpy(&ctx->tick, p, sizeof(uint64_t));
    p += sizeof(uint64_t);
    memcpy(&ctx->context_len, p, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(&ctx->context_pos, p, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(&ctx->history_len, p, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(&ctx->history_pos, p, sizeof(size_t));
    p += sizeof(size_t);
    memcpy(ctx->context, p, KWM_CONTEXT_SIZE);
    p += KWM_CONTEXT_SIZE;
    memcpy(ctx->history, p, KWM_HISTORY_SIZE);
    p += KWM_HISTORY_SIZE;
    memcpy(&ctx->stats, p, sizeof(KwmStats));
    p += sizeof(KwmStats);
    memcpy(&ctx->learning_rate, p, sizeof(float));
    p += sizeof(float);
    memcpy(&ctx->surprise_threshold, p, sizeof(float));
    p += sizeof(float);
    memcpy(&ctx->auto_learn, p, sizeof(int));
    p += sizeof(int);

    size_t concept_count;
    memcpy(&concept_count, p, sizeof(size_t));
    p += sizeof(size_t);
    if (concept_count > KWM_MAX_CONCEPTS)
        concept_count = KWM_MAX_CONCEPTS;
    ctx->concept_count = concept_count;
    memcpy(ctx->concepts, p, concept_count * sizeof(KwmConcept));
    p += concept_count * sizeof(KwmConcept);

    /* Бэкбон */
    int rc = kat_deserialize((KatModel *)ctx->backbone, p, buf_size - (size_t)(p - buf));
