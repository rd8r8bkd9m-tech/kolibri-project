...
static const char *logic_keywords[] = {"задач", "реши", "логическ", "докажи", "головоломк"};
static const char *math_keywords[] = {"посчитай", "вычисли", "реши", "уравнен", "формул", "сколько будет", "интеграл", "дифференци"};
...
int kolibri_ic_classify(KolibriIntentClassifier *classifier, const char *query, KolibriIntentResult *result) {
    ...
    for (int i = 0; i < classifier->num_patterns; i++) {
        ...
        if (matches > 0) {
            /* Keywords are alternatives, so any match gives base confidence */
            double score = pattern->base_confidence;
            /* Slight boost for multiple matches */
            if (matches > 1) score += 0.05 * (matches - 1);
