...
int kolibri_ic_init(KolibriIntentClassifier *classifier) {
    ...
    /* Добавляем паттерны */
    kolibri_ic_add_pattern(classifier, KIC_INTENT_QUERY_FACT, 
                           fact_keywords, sizeof(fact_keywords)/sizeof(fact_keywords[0]),
                           0.7, "general");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_QUERY_DEFINITION,
                           def_keywords, sizeof(def_keywords)/sizeof(def_keywords[0]),
                           0.8, "general");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_QUERY_COMPARISON,
                           compare_keywords, sizeof(compare_keywords)/sizeof(compare_keywords[0]),
                           0.85, "general");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_QUERY_CAUSE,
                           cause_keywords, sizeof(cause_keywords)/sizeof(cause_keywords[0]),
                           0.8, "general");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_QUERY_PROCESS,
                           process_keywords, sizeof(process_keywords)/sizeof(process_keywords[0]),
                           0.8, "science");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_LOGIC_PUZZLE,
                           logic_keywords, sizeof(logic_keywords)/sizeof(logic_keywords[0]),
                           0.75, "logic");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_MATH_PROBLEM,
                           math_keywords, sizeof(math_keywords)/sizeof(math_keywords[0]),
                           0.8, "math");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_EXPLAIN,
                           explain_keywords, sizeof(explain_keywords)/sizeof(explain_keywords[0]),
                           0.7, "general");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_EXAMPLE,
                           example_keywords, sizeof(example_keywords)/sizeof(example_keywords[0]),
                           0.75, "general");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_COUNTERFACTUAL,
                           whatif_keywords, sizeof(whatif_keywords)/sizeof(whatif_keywords[0]),
                           0.9, "counterfactual");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_TEACH,
                           teach_keywords, sizeof(teach_keywords)/sizeof(teach_keywords[0]),
                           0.85, "learning");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_GREETING,
                           greet_keywords, sizeof(greet_keywords)/sizeof(greet_keywords[0]),
                           0.9, "conversation");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_FAREWELL,
                           farewell_keywords, sizeof(farewell_keywords)/sizeof(farewell_keywords[0]),
                           0.9, "conversation");
    
    kolibri_ic_add_pattern(classifier, KIC_INTENT_THANKS,
                           thanks_keywords, sizeof(thanks_keywords)/sizeof(thanks_keywords[0]),
                           0.9, "conversation");
...
int kolibri_ic_classify(KolibriIntentClassifier *classifier,
    ...
    if (!classifier || !query || !result) return -1;
    ...
    for (int i = 0; i < classifier->num_patterns; i++) {
        ...
        int matches = count_keyword_matches(lower_query, 
                                           pattern->keywords, 
                                           pattern->num_keywords);
    ...
    if (best_idx >= 0 && best_score >= KIC_MIN_CONFIDENCE) {
        ...
        result->requires_knowledge = (result->primary_intent == KIC_INTENT_QUERY_FACT ||
                                      result->primary_intent == KIC_INTENT_QUERY_DEFINITION ||
...
int kolibri_ic_add_pattern(KolibriIntentClassifier *classifier,
                           ...
                           const char *domain) {
    if (!classifier || classifier->num_patterns >= KIC_MAX_PATTERNS) return -1;
...
const char* kolibri_ic_intent_name(KolibriIntent intent) {
    ...
    if (intent < 0 || intent > KIC_INTENT_UNKNOWN) return "INVALID";
    return names[intent];
}

const char* kolibri_ic_intent_desc(KolibriIntent intent) {
    static const char *descs[] = {
        "Запрос факта (сколько, когда, где)",
    ...
    if (intent < 0 || intent > KIC_INTENT_UNKNOWN) return "Неверный intent";
    return descs[intent];
}
...
void kolibri_ic_print_result(const KolibriIntentResult *result) {
    ...
    if (result->num_hypotheses > 1) {
        ...
        for (int i = 0; i < result->num_hypotheses; i++) {
            printf("  %d. %s (%.2f)\n", 
                   i + 1,
                   kolibri_ic_intent_name(result->top_intents[i]),
...
int kolibri_ic_requires_reasoning(KolibriIntent intent) {
    return (intent == KIC_INTENT_LOGIC_PUZZLE ||
            intent == KIC_INTENT_MATH_PROBLEM ||
            intent == KIC_INTENT_DEDUCTION ||
            intent == KIC_INTENT_COUNTERFACTUAL ||
            intent == KIC_INTENT_QUERY_CAUSE ||
