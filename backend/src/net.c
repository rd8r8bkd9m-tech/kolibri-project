...
#include "kolibri/evolutionary_trainer.h" /* KET_FORMULA_SIZE */
...
#define KOLIBRI_MAX_PAYLOAD 4096U /* Увеличено с 256 для поддержки полных формул (4000 байт) */

static uint64_t kolibri_htonll(uint64_t value) {
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
    return (((uint64_t)htonl((uint32_t)(value & 0xFFFFFFFFULL))) << 32) | htonl((uint32_t)(value >> 32));
#else
    return value;
#endif
}
...
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
    return (((uint64_t)ntohl((uint32_t)(value & 0xFFFFFFFFULL))) << 32) | ntohl((uint32_t)(value >> 32));
#else
    return value;
#endif
}

static size_t kolibri_write_header(uint8_t *buffer, size_t buffer_len, KolibriNetMessageType type,
...
static int kolibri_recv_all(int sockfd, uint8_t *data, size_t len) {
    ...
    while (received_total < len) {
        ssize_t received = recv(sockfd, data + received_total, len - received_total, 0);
        if (received < 0) {
            if (errno == EINTR) {
                continue;
            }
...
size_t kn_message_encode_hello(uint8_t *buffer, size_t buffer_len, uint32_t node_id) {
    ...
    size_t header = kolibri_write_header(buffer, buffer_len, KOLIBRI_MSG_HELLO, sizeof(payload));
...
size_t kn_message_encode_formula(uint8_t *buffer, size_t buffer_len, uint32_t node_id, const KolibriFormula *formula) {
    ...
    uint8_t digits[KET_FORMULA_SIZE]; /* Полная формула 4000 байт, не обрезаем */
    ...
    /* 2-байтовая длина для поддержки формул до 65535 байт */
    uint16_t be_dlen = htons((uint16_t)digit_len);
    memcpy(payload + offset, &be_dlen, sizeof(be_dlen));
    offset += sizeof(be_dlen);
    memcpy(payload + offset, digits, digit_len);
    offset += digit_len;
    memcpy(payload + offset, &fitness_bits, sizeof(fitness_bits));
    offset += sizeof(fitness_bits);

    size_t header = kolibri_write_header(buffer, buffer_len, KOLIBRI_MSG_MIGRATE_RULE, offset);
...
size_t kn_message_encode_knowledge(uint8_t *buffer, size_t buffer_len, const char *q, const char *a) {
    ...
    size_t offset = 0;
    memcpy(payload + offset, &be_ih, 4);
    offset += 4;
    memcpy(payload + offset, &be_oh, 4);
    offset += 4;
    payload[offset++] = (uint8_t)qlen;
    memcpy(payload + offset, q, qlen);
    offset += qlen;
    payload[offset++] = (uint8_t)alen;
    memcpy(payload + offset, a, alen);
    offset += alen;
...
size_t kn_message_encode_ack(uint8_t *buffer, size_t buffer_len, uint8_t status) {
    ...
    if (header == 0 || buffer_len < header + 1)
        return 0;
...
#define PACK_U64(val)                                                                                                  \
    do {                                                                                                               \
        tmp = kolibri_htonll(val);                                                                                     \
        memcpy(payload + offset, &tmp, 8);                                                                             \
        offset += 8;                                                                                                   \
    } while (0)
#define PACK_DBL(val)                                                                                                  \
    do {                                                                                                               \
        memcpy(&tmp, &val, 8);                                                                                         \
        tmp = kolibri_htonll(tmp);                                                                                     \
        memcpy(payload + offset, &tmp, 8);                                                                             \
        offset += 8;                                                                                                   \
    } while (0)
...
size_t kn_message_encode_mutation_req(uint8_t *buffer, size_t buffer_len, uint32_t node_id, uint8_t mutation_type) {
    ...
    if (header == 0 || buffer_len < header + 5)
        return 0;
    memcpy(buffer + header, payload, 5);
    return header + 5;
}

size_t kn_message_encode_mutation_resp(uint8_t *buffer, size_t buffer_len, uint32_t node_id, const uint8_t *digits,
                                       uint8_t length) {
    if (!buffer || !digits || length > 32)
        return 0;
    uint8_t payload[4 + 1 + 32];
    uint32_t be_id = htonl(node_id);
    size_t offset = 0;
    memcpy(payload + offset, &be_id, 4);
    offset += 4;
    payload[offset++] = length;
    memcpy(payload + offset, digits, length);
    offset += length;
...
size_t kn_message_encode_divergence(uint8_t *buffer, size_t buffer_len, uint32_t node_id, uint64_t expected_hash,
    ...
    memcpy(payload + offset, &be_id, 4);
    offset += 4;
    memcpy(payload + offset, &be_exp, 8);
    offset += 8;
    memcpy(payload + offset, &be_act, 8);
    offset += 8;
...
int kn_message_decode(const uint8_t *buffer, size_t buffer_len, KolibriNetMessage *out_message) {
    ...
    case KOLIBRI_MSG_MIGRATE_RULE: {
        if (payload_len < sizeof(uint32_t) + sizeof(uint16_t) + sizeof(uint64_t)) {
            return -1;
        }
        size_t offset = 0;
        uint32_t node_raw;
        ...
        /* 2-байтовая длина для согласованности с энкодером */
        uint16_t length_be;
        memcpy(&length_be, payload + offset, sizeof(length_be));
        offset += sizeof(length_be);
        uint16_t length = ntohs(length_be);
        if (length > KET_FORMULA_SIZE || payload_len < offset + length + sizeof(uint64_t)) {
            return -1;
        }
        memset(out_message->data.formula.digits, 0, sizeof(out_message->data.formula.digits));
    ...
    case KOLIBRI_MSG_FITNESS_RESP: {
        ...
        size_t offset = 0;
        uint64_t tmp;
#define UNPACK_U64(field)                                                                                              \
    do {                                                                                                               \
        memcpy(&tmp, payload + offset, 8);                                                                             \
        field = kolibri_ntohll(tmp);                                                                                   \
        offset += 8;                                                                                                   \
    } while (0)
#define UNPACK_DBL(field)                                                                                              \
    do {                                                                                                               \
        memcpy(&tmp, payload + offset, 8);                                                                             \
        tmp = kolibri_ntohll(tmp);                                                                                     \
        memcpy(&field, &tmp, 8);                                                                                       \
        offset += 8;                                                                                                   \
    } while (0)
...
#undef UNPACK_DBL
    ...
    case KOLIBRI_MSG_MUTATION_RESP: {
        ...
        if (payload_len < (size_t)(5 + out_message->data.mutation_resp.length))
            return -1;
...
int kn_request_fitness(const char *host, uint16_t port, uint32_t node_id, KolibriEvolutionMetrics *out_metrics) {
    ...
    if (kn_send_message(sockfd, buffer, len) < 0) {
        close(sockfd);
        return -1;
    }
    ...
    if (payload_len > sizeof(recv_buf) - KOLIBRI_HEADER_SIZE) {
        close(sockfd);
        return -1;
    }
    ...
    if (kn_message_decode(recv_buf, KOLIBRI_HEADER_SIZE + payload_len, &msg) < 0) {
        close(sockfd);
        return -1;
    }

    if (msg.type != KOLIBRI_MSG_FITNESS_RESP) {
        close(sockfd);
        return -1;
    }
...
int kn_send_divergence_report(const char *host, uint16_t port, uint32_t node_id, uint64_t expected_hash,
                              uint64_t actual_hash) {
    if (!host)
        return -1;
    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
...
int kn_listener_poll(KolibriNetListener *listener, uint32_t timeout_ms, KolibriNetMessage *out_message) {
    ...
    int client_fd = accept(listener->socket_fd, (struct sockaddr *)&client_addr, &client_len);
    if (client_fd < 0) {
