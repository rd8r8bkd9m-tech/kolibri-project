...
KolibriEqType kolibri_parse_equation(const char *equation,
    ...
    if (!equation || !a || !b || !c) return KMS_EQ_UNKNOWN;

    *a = *b = *c = 0;

    /* Упрощённый парсер — в будущем будет полноценный */
    /* Пока поддерживаем форматы:
       "ax + b = c"
       "ax^2 + bx + c = 0"
       "ax² + bx + c = 0"
    */

    /* Проверяем наличие x^2 или x² */
    int has_square = (strstr(equation, "x^2") != NULL ||
                     strstr(equation, "x²") != NULL);

    if (has_square) {
        *a = 1; *b = 0; *c = 0;

        /* Ищем коэффициенты (очень упрощённо) */
        /* В полноценной версии здесь будет парсер выражений */
        sscanf(equation, "%lfx^2", a);
        if (a[0] == 0) *a = 1;
