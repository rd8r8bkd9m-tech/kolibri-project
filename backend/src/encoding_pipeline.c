...
int kolibri_pipeline_encode_word(KolibriEncodingPipeline *pipeline, const char *word, KolibriEncodingResult *result) {
    ...
    if (pipeline->config.enable_digits) {
        static uint8_t digit_buf[4096];
        kolibri_potok_cifr_init(&result->digit_stream, digit_buf, sizeof(digit_buf));
    ...
    if (pipeline->config.enable_semantic) {
        ...
        if (existing >= 0) {
            ...
            if (k_semantic_context_init(&ctx) == 0) {
                /* Build minimal context: just the word itself as context */
                static uint8_t ctx_digit_buf[4096];
                kolibri_potok_cifr_init(&ctx.word_digits, ctx_digit_buf, sizeof(ctx_digit_buf));
