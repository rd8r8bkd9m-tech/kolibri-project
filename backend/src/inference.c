...
static void inference_digit_vote_add(InferenceDigitVoteAccumulator *acc,
                                     size_t digit,
                                     double value) {
...
static void inference_fill_query_semantics(const char *query,
                                           KolibriQuerySemanticSummary *summary) {
    if (!summary) {
        return;
    }
    memset(summary, 0, sizeof(*summary));
    ...
    inference_extract_definition_entity(normalized_query,
                                        summary->definition_entity,
                                        sizeof(summary->definition_entity));

    char topic_tokens[8][128];
    summary->topic_token_count =
        inference_extract_topic_tokens(normalized_query, topic_tokens, 8U);
    if (summary->definition_entity[0] != '\0') {
        snprintf(summary->canonical_topic,
                 sizeof(summary->canonical_topic),
                 "%s",
                 summary->definition_entity);
...
static int inference_delta_encode(const uint8_t *digits, size_t len,
                                  int8_t *delta_out, size_t delta_len);
static double inference_delta_similarity(const int8_t *delta_a,
                                         const int8_t *delta_b, size_t len);
static double inference_position_decay(size_t position, size_t total);
static int inference_evolve_formulas(const KolibriFormulaPool *pool,
                                     const char *query,
                                     KolibriAssociation **best_assoc,
                                     double *best_score);
static int inference_mutate_query(const char *query, char *mutated, size_t mutated_size);
static int inference_synthesize_answer(const char *query,
                                       const KolibriFormulaPool *pool,
                                       char *output, size_t output_size);
static int inference_path_mutation(const uint8_t *original_path, size_t path_len,
                                   uint8_t *mutated_path, size_t mutated_size);
...
static int step_direct_search(
    ...
    int has_delta = (query_digits_len > 0 && query_digits_len < 1024) ?
                     (inference_delta_encode(query_digits, query_digits_len,
                                             query_delta, 1024) == 0) : 0;
    ...
    for (size_t i = 0; i < found && i < 16; i++) {
        ...
        if (has_delta && results[i]->content) {
            ...
            size_t doc_len = inference_encode_text_digits(&g_formula_symbol_table,
                                                           results[i]->content,
                                                           doc_digits, sizeof(doc_digits));
            int8_t doc_delta[1024];
            if (doc_len > 0 && doc_len < 1024 &&
                inference_delta_encode(doc_digits, doc_len, doc_delta, 1024) == 0) {
    ...
    for (size_t i = 0; i < found; i++) {
        for (size_t j = i + 1; j < found; j++) {
            if (scores[j] > scores[i]) {
                double ts = scores[i]; scores[i] = scores[j]; scores[j] = ts;
                const KolibriKnowledgeDocument *td = results[i]; results[i] = results[j]; results[j] = td;
            }
        }
    }
    ...
    if (found > 0 && scores[0] > 0.0) {
        ...
        *source_count = found;
        ...
        snprintf(step->result, sizeof(step->result),
                 "Найдено %zu релевантных документов (лучший балл: %.2f, дельта-boost применён)",
                 found, scores[0]);
    } else {
        snprintf(step->result, sizeof(step->result),
...
retry_formula_search:
    ...
    for (size_t i = 0; i < pool->association_count; ++i) {
        ...
        inference_digit_vote_add(&vote_acc, 5U,
                                 inference_text_overlap_score(active_query, normalized_assoc) * 2.5);
        inference_digit_vote_add(&vote_acc, 4U,
                                 inference_digit_similarity(query_digits,
                                                            query_digits_len,
                                                            assoc->question_digits,
                                                            assoc->question_digits_length) * 1.5);
        inference_digit_vote_add(&vote_acc, 9U,
                                 inference_text_overlap_score(active_query, assoc->answer) * 0.45);
        {
        ...
        if (strict_mode && candidate_topic_count > query_topic_count &&
            ...
            inference_digit_vote_add(&vote_acc, 0U,
                                     (double)(candidate_topic_count - query_topic_count) * 0.9);
        }
    ...
    if (best_assoc && best_score >= 1.15 && best_assoc->answer[0] != '\0') {
        ...
        for (size_t i = 0; i < pool->association_count && top3_count < 3; ++i) {
            const KolibriAssociation *assoc = &pool->associations[i];
            if (assoc == best_assoc || assoc->answer[0] == '\0') continue;
        ...
        if (top3_count >= 2) {
            ...
            snprintf(step->result, sizeof(step->result),
                     "Ensemble: %zu ассоциаций (балл: %.2f, consensus)",
                     top3_count, total_weight);
        } else {
            ...
            snprintf(step->result, sizeof(step->result),
                     ...
                     best_score,
                     (unsigned)best_vote_summary.winner_digit,
                     best_vote_summary.consensus);
        }
        if (formulas_applied) {
        ...
        inference_evolve_formulas(pool, normalized_query,
                                  &evolved_assoc, &evolved_score);

        if (evolved_assoc && evolved_score > 0.3) {
            inference_compose_human_answer(query,
                                           evolved_assoc->answer,
                                           partial_response,
                                           partial_size);
            step->confidence = evolved_score / 4.0;
            snprintf(step->result, sizeof(step->result),
                     "Эволюция формул: найдена ассоциация (балл: %.2f после %d поколений)",
                     evolved_score, 4);
            if (formulas_applied) *formulas_applied = 1U;
        } else {
            /* #7. Мутация при низком confidence */
            char mutated_query[1024];
            int mutation_ok = inference_mutate_query(normalized_query,
                                                     mutated_query,
                                                     sizeof(mutated_query));
            if (mutation_ok == 0) {
                ...
                for (size_t i = 0; i < pool->association_count && best_score < 1.15; ++i) {
                    ...
                    if (abs(mutated_hash - assoc_hash) < 500 && assoc->answer[0] != '\0') {
                        inference_compose_human_answer(query, assoc->answer,
                                                       partial_response, partial_size);
                        best_score = 0.8;
                        best_assoc = assoc;
                        if (formulas_applied) *formulas_applied = 1U;
                        break;
            ...
            if (best_score < 1.15 && partial_response[0] == '\0') {
                inference_synthesize_answer(normalized_query, pool,
                                            partial_response, partial_size);
                if (partial_response[0] != '\0') {
                    step->confidence = 0.3;
                    snprintf(step->result, sizeof(step->result),
                             "Синтез ответа через формулы (приближённый)");
                    if (formulas_applied) *formulas_applied = 1U;
                } else {
                    snprintf(step->result, sizeof(step->result),
...
static int multihop_search(const KolibriFormulaPool *pool,
                            const char *current_entity,
                            const char *target_hint,
                            int depth,
                            MultiHopChain *chain,
                            int *visited_hashes,
                            int visited_count) {
    if (depth >= KOLIBRI_MULTIHOP_MAX_DEPTH) return 0;
    if (!pool || !current_entity || !chain) return 0;
    ...
    for (int i = 0; i < visited_count; i++) {
        if (visited_hashes[i] == entity_hash) return 0;
    }
    if (visited_count >= 64) return 0;
    ...
    for (size_t i = 0; i < pool->association_count; i++) {
        ...
        if (strstr(assoc->question, current_entity) == NULL) continue;

        /* Извлекаем отношение и цель */
        char *arrow = strstr(assoc->question, "->");
        if (!arrow) arrow = strstr(assoc->question, "→");
        ...
        if (assoc->answer[0] != '\0') edge_confidence += 0.2;
        if (strlen(assoc->source) > 0) edge_confidence += 0.1;

        /* Добавляем ребро в цепочку */
        int edge_idx = chain->path_length;
        if (edge_idx >= KOLIBRI_MULTIHOP_MAX_DEPTH) continue;
        ...
        found_chains += multihop_search(pool, target, target_hint,
                                         depth + 1, chain,
        ...
        if (found_chains >= KOLIBRI_MULTIHOP_MAX_BRANCH) break;
    }
...
static int step_logical_reasoning(
    ...
    double t0 = now_ms();
    snprintf(step->description, sizeof(step->description),
             "Multi-hop reasoning (глубина %d)", KOLIBRI_MULTIHOP_MAX_DEPTH);
    ...
    if (!pool || pool->association_count == 0) {
        ...
        for (size_t i = 0; i < mem->cell_count && chain_depth < 3; i++) {
            if (mem->cells[i].logic &&
                mem->cells[i].logic->type == LOGIC_COMPOSITION) {
                chain_depth++;
            }
        }
    ...
    if (entity[0] == '\0') {
        snprintf(step->result, sizeof(step->result),
                 "Не удалось извлечь сущность для multi-hop");
        step->confidence = 0.1;
        step->duration_ms = now_ms() - t0;
        return 0;
    }
    ...
    int found = multihop_search(pool, entity, NULL, 0, &chain,
                                 visited_hashes, 0);
    ...
    *rules_fired = (size_t)found;
    ...
    if (found > 0 && chain.path_length > 0) {
        ...
        for (int i = 0; i < chain.path_length && pos < sizeof(reasoning_text) - 50; i++) {
            ...
            pos += snprintf(reasoning_text + pos, sizeof(reasoning_text) - pos,
                           "%s", chain.path[i].target);
        }

        snprintf(step->result, sizeof(step->result),
                 "Multi-hop: глубина=%d, цепочек=%d, уверенность=%.2f: %s",
                 chain.path[chain.path_length - 1].depth + 1,
                 found, chain.total_confidence, reasoning_text);

        step->confidence = chain.total_confidence;
    } else {
        snprintf(step->result, sizeof(step->result),
                 "Multi-hop: цепочки не найдены для сущности \"%s\"", entity);
        step->confidence = 0.2;
    }
...
static int step_fractal_memory(
    ...
    if (found > 0 && results[0].similarity > 0.3f) {
        ...
        if (results[0].path_len >= 5) depth_confidence = 0.8;
        else if (results[0].path_len >= 3) depth_confidence = 0.6;
        else depth_confidence = 0.3;

        step->confidence = ((double)results[0].similarity + depth_confidence) / 2.0;
        snprintf(step->result, sizeof(step->result),
                 "Найдено %d фрактальных путей (близость: %.2f, глубина: %u, уверенность глубины: %.1f)",
                 found, results[0].similarity, results[0].path_len, depth_confidence);
    } else {
        ...
        for (int depth = 0; depth < 5 && energy > 0.1; depth++) {
            ...
            if (spread_found > 0 && spread_results[0].similarity > 0.2f) {
                if (spread_results[0].node && spread_results[0].node->payload_size > 0) {
                    ...
                    snprintf(step->result, sizeof(step->result),
                             "Spreading activation: глубина=%d, энергия=%.2f, путь=%zu",
                             depth + 1, energy, active_len);
        ...
        if (inference_path_mutation(query_path, query_path_len,
                                     mutated_path, KFM_MAX_DEPTH) == 0) {
            ...
            if (mut_found > 0 && mut_results[0].similarity > 0.25f) {
                if (mut_results[0].node && mut_results[0].node->payload_size > 0) {
                    ...
                    snprintf(step->result, sizeof(step->result),
                             "Path mutation: найдено при близости %.2f",
...
static void inference_consensus_add(InferenceDecimalConsensus *consensus,
                                    const KolibriNumericVoteSummary *vote);
...
static int inference_compute_attention(const char *query,
                                       InferenceAttentionToken *tokens,
...
static int inference_transitive_reasoning(const char *query, const KolibriFormulaPool *pool,
                                          char *output, size_t output_size);
static int inference_world_model_check(const char *query, const char *answer,
                                       char *validated, size_t validated_size);
static int inference_validate_answer(const char *query, const char *answer,
...
int kolibri_inference_run(
    ...
    KolibriInferenceStrategy effective_strategy = ctx->strategy;
    if (effective_strategy == KOLIBRI_INF_HYBRID || effective_strategy == KOLIBRI_INF_DIRECT) {
        effective_strategy = inference_select_strategy(query);
    }
    ...
    if (effective_strategy == KOLIBRI_INF_DIRECT ||
        effective_strategy == KOLIBRI_INF_HYBRID ||
        effective_strategy == KOLIBRI_INF_CHAIN) {
        step_direct_search(query, &result->steps[step_idx],
                          direct_response, sizeof(direct_response), &source_count);
    ...
    if (effective_strategy == KOLIBRI_INF_FORMULA ||
        effective_strategy == KOLIBRI_INF_HYBRID ||
        effective_strategy == KOLIBRI_INF_CHAIN) {
        step_formula_inference(query, &result->steps[step_idx],
                              formula_response, sizeof(formula_response),
    ...
    if (effective_strategy == KOLIBRI_INF_LOGICAL ||
        effective_strategy == KOLIBRI_INF_HYBRID) {
    ...
    if (effective_strategy == KOLIBRI_INF_HYBRID ||
        effective_strategy == KOLIBRI_INF_CHAIN) {
        step_fractal_memory(query, &result->steps[step_idx],
                           fractal_response, sizeof(fractal_response));
        step_idx++;
    }

    if (effective_strategy == KOLIBRI_INF_HYBRID ||
        effective_strategy == KOLIBRI_INF_CHAIN) {
        step_semantic_understanding(query, &result->steps[step_idx],
                                   semantic_response, sizeof(semantic_response));
        step_idx++;
    }

    if (effective_strategy == KOLIBRI_INF_CHAIN ||
        (effective_strategy == KOLIBRI_INF_HYBRID && strlen(query) > 50)) {
        size_t cot_steps = 0;
        size_t cot_max = KOLIBRI_INF_MAX_STEPS - step_idx;
        if (cot_max > 4) cot_max = 4;
        step_chain_of_thought(query, &result->steps[step_idx], cot_max,
                             &cot_steps, cot_response, sizeof(cot_response));
        step_idx += cot_steps;
    }
    ...
    const KolibriFormulaPool *pool = inference_get_formula_memory();
    if (pool && pool->association_count > 0) {
        inference_transitive_reasoning(query, pool,
                                       transitive_response, sizeof(transitive_response));
    }
    ...
    if (n_candidates > 0) {
        ...
        for (size_t i = 0; i < n_candidates; i++) {
            ...
            if (w > best_weight) { best_weight = w; best_idx = (int)i; }
        }
        ...
        if (strlen(formula_response) > 0 && best_idx != 1) {
            strncpy(consensus_candidates[1].response, formula_response,
                    sizeof(consensus_candidates[1].response) - 1);
        ...
        for (int i = 1; i < n_consensus; i++) {
            ...
            strncpy(temp1, resp1, sizeof(temp1) - 1); temp1[sizeof(temp1) - 1] = '\0';
            strncpy(temp2, resp2, sizeof(temp2) - 1); temp2[sizeof(temp2) - 1] = '\0';
            ...
            if (common_words >= 3) agreement_count++;
        }

        double consensus_boost = 1.0;
        if (agreement_count >= 4) consensus_boost = 1.3;
        else if (agreement_count >= 3) consensus_boost = 1.2;
        else if (agreement_count >= 2) consensus_boost = 1.1;

        /* #Фаза 4.2: Verification step */
        char verified[4096] = {0};
        inference_world_model_check(query, consensus_candidates[0].response,
        ...
        if (contradiction) {
            snprintf(result->response, KOLIBRI_INF_MAX_RESPONSE,
                     "%s (возможны противоречия, консенсус: %d/%d)",
            ...
            if (result->total_confidence > 1.0) result->total_confidence = 1.0;
        }
        ...
        if (agreement_count >= 3) {
            ...
            if (len < KOLIBRI_INF_MAX_RESPONSE - 50) {
                snprintf(result->response + len, KOLIBRI_INF_MAX_RESPONSE - len,
                         " [консенсус: %d/%d стратегий]", agreement_count, n_consensus);
            }
        }

        if (vs) mf_destroy_store(vs);
...
int kolibri_inference_feedback(KolibriInferenceContext *ctx,
                                 const char *query,
                                 const char *response,
                                 double feedback_score) {
    if (!ctx || !query || !response) return -1;
    ...
    for (size_t i = 0; i < pool->count; i++) {
        ...
        for (size_t j = 0; j < formula->association_count; j++) {
            ...
            if (strstr(assoc->question, query) != NULL ||
                abs(kf_hash_from_text(assoc->question) - query_hash) < 1000) {

                double delta = feedback_score - formula->fitness;
                formula->fitness += delta * 0.1;

                if (formula->fitness < 0.0) formula->fitness = 0.0;
                if (formula->fitness > 1.0) formula->fitness = 1.0;
...
int kolibri_inference_get_metrics(const KolibriInferenceContext *ctx,
                                   KolibriInferenceMetrics *metrics) {
...
static int inference_build_query_genome(const char *query,
                                        KolibriQueryGenome *genome) {
    if (!query || !genome) return -1;
    ...
    uint8_t digits[1024];
    size_t digits_len = inference_encode_text_digits(&g_formula_symbol_table,
                                                      normalized,
                                                      digits,
                                                      sizeof(digits));
    free(normalized);
...
static double inference_genome_similarity(const KolibriQueryGenome *a,
                                          const KolibriQueryGenome *b) {
...
static int inference_delta_encode(const uint8_t *digits, size_t len,
                                  int8_t *delta_out, size_t delta_len) {
    if (!digits || !delta_out || len == 0 || delta_len < len) return -1;
...
static double inference_delta_similarity(const int8_t *delta_a,
                                         const int8_t *delta_b,
                                         size_t len) {
    if (!delta_a || !delta_b || len == 0) return 0.0;
...
static void inference_cache_insert(uint64_t answer_hash, const char *answer_text, double score) {
    ...
    if (g_answer_cache_count > 0) {
        memmove(&g_answer_cache[1], &g_answer_cache[0],
                g_answer_cache_count * sizeof(InferenceAnswerCacheEntry));
    }
...
static KolibriInferenceStrategy inference_select_strategy(const char *query) {
    if (!query) return KOLIBRI_INF_HYBRID;
    ...
    if (!lower) return KOLIBRI_INF_HYBRID;

    int is_definition = inference_is_definition_query(lower);
    int is_comparison = inference_contains_phrase(lower, "сравни") ||
                        inference_contains_phrase(lower, "разница") ||
                        inference_contains_phrase(lower, "отличие");
    int is_calculation = inference_contains_phrase(lower, "сколько") ||
                         inference_contains_phrase(lower, "посчитай") ||
                         inference_contains_phrase(lower, "вычисли");
    int is_explain = inference_contains_phrase(lower, "почему") ||
                     inference_contains_phrase(lower, "как работает") ||
                     inference_contains_phrase(lower, "объясни");
    int is_list = inference_contains_phrase(lower, "перечисли") ||
                  inference_contains_phrase(lower, "какие") ||
                  inference_contains_phrase(lower, "список");
    free(lower);

    /* Адаптивный выбор */
    if (is_definition) return KOLIBRI_INF_FORMULA;
    if (is_comparison) return KOLIBRI_INF_HYBRID;
    if (is_calculation) return KOLIBRI_INF_FORMULA;
    if (is_explain) return KOLIBRI_INF_CHAIN;
    if (is_list) return KOLIBRI_INF_DIRECT;

    /* Fallback: числовой выбор */
    switch (h % 7) {
        case 0: return KOLIBRI_INF_DIRECT;
        case 1: return KOLIBRI_INF_FORMULA;
        case 2: return KOLIBRI_INF_LOGICAL;
        case 3: return KOLIBRI_INF_CHAIN;
        case 4: case 5: case 6: return KOLIBRI_INF_HYBRID;
        default: return KOLIBRI_INF_HYBRID;
    }
}
...
static int inference_evolve_formulas(const KolibriFormulaPool *pool,
                                     ...
                                     KolibriAssociation **best_assoc,
                                     double *best_score) {
    if (!pool || !query || !best_assoc || !best_score) return -1;
    if (pool->association_count < 4) return -1;
    ...
    for (int gen = 0; gen < EVOLVE_GEN; gen++) {
        ...
        for (int p = 0; p < EVOLVE_POP; p++) {
            ...
            if (!norm_assoc) { fitness[p] = 0.0; continue; }
        ...
        for (int e = 0; e < 4; e++) {
            ...
            for (int j = 1; j < EVOLVE_POP; j++) {
                if (fitness[j] > fitness[best_idx]) best_idx = j;
            }
...
static int inference_crossover_associations(const KolibriAssociation *a,
                                            const KolibriAssociation *b,
                                            char *output, size_t output_size) {
    if (!a || !b || !output || output_size == 0) return -1;
    ...
    if (a_take == 0) a_take = a_len / 2;
    else a_take++; /* Включаем точку */
    ...
    while (b_start < b_len && b_start < b_take) {
        if (b->answer[b_start] == '.' || b->answer[b_start] == '!' || b->answer[b_start] == '?') {
            b_start++;
            if (b_start < b_len && b->answer[b_start] == ' ') b_start++;
            break;
        }
        b_start++;
    }
...
static int inference_mutate_query(const char *query, char *mutated, size_t mutated_size) {
    if (!query || !mutated || mutated_size == 0) return -1;
...
static int inference_synthesize_answer(const char *query,
                                       const KolibriFormulaPool *pool,
                                       char *output, size_t output_size) {
    if (!query || !pool || !output || output_size == 0) return -1;

    /* Кодируем запрос в цифры */
    uint8_t query_digits[256];
    size_t query_len = inference_encode_text_digits(&g_formula_symbol_table,
                                                     query, query_digits,
                                                     sizeof(query_digits));
...
static double inference_spreading_activation(const char *query,
                                             void *fmem,
                                             char *answer, size_t answer_size) {
    if (!query || !fmem || !answer || answer_size == 0) return 0.0;
    ...
    /* #11. Глубина = уверенность */
    double depth_confidence = 0.0;
    if (best_depth >= 5) depth_confidence = 0.8;
    else if (best_depth >= 3) depth_confidence = 0.6;
    else if (best_depth >= 1) depth_confidence = 0.4;
    else depth_confidence = 0.1;

    if (max_energy > 0.1) {
        snprintf(answer, answer_size,
                 "Фрактальная память: энергия=%.2f, глубина=%d, уверенность=%.1f",
                 max_energy, best_depth, depth_confidence);
...
static int inference_path_mutation(const uint8_t *original_path, size_t path_len,
                                   uint8_t *mutated_path, size_t mutated_size) {
    if (!original_path || !mutated_path || path_len == 0 || mutated_size < path_len) return -1;
...
static int inference_transitive_reasoning(const char *query,
                                          const KolibriFormulaPool *pool,
                                          char *output, size_t output_size) {
    if (!query || !pool || !output || output_size == 0) return -1;
    ...
    if (bc) {
        ...
        snprintf(output, output_size,
                 "Транзитивный вывод: %s → %s → %s",
                 entity, ab->answer, bc->answer);
        return 0;
    }
...
static int inference_recursive_materialize(const char *pattern, int count,
                                           char *output, size_t output_size) {
    if (!pattern || !output || output_size == 0 || count <= 0) return -1;
...
static int inference_validate_answer(const char *query, const char *answer,
                                     MetaFormulaStore *store) {
    if (!query || !answer || !store) return 0;
    ...
    int query_negated = inference_contains_phrase(q_lower, "не ") ||
                        inference_contains_phrase(q_lower, "нет ");
...
static int inference_compute_attention(const char *query,
                                       ...
                                       size_t max_tokens,
                                       size_t *out_count) {
    if (!query || !tokens || max_tokens == 0 || !out_count) return -1;
    ...
    /* Вычисляем attention: softmax по длине токена (длинные = важные) */
    double sum_exp = 0.0;
    for (size_t i = 0; i < n; i++) {
        double score = (double)strlen(tokens[i].token) / 5.0;
        tokens[i].weight = exp(score);
...
static int inference_world_model_check(const char *query, const char *answer,
                                       char *validated, size_t validated_size) {
    if (!query || !answer || !validated || validated_size == 0) return -1;

    /* Простая проверка: если ответ содержит факты, проверяем их */
    char *a_lower = inference_lower_ascii_copy(answer);
    if (!a_lower) return -1;

    /* Проверяем на «неподтверждённые» утверждения */
    int has_uncertain = inference_contains_phrase(a_lower, "возможно") ||
                        inference_contains_phrase(a_lower, "вероятно") ||
                        inference_contains_phrase(a_lower, "может быть");

    if (has_uncertain) {
        snprintf(validated, validated_size, "По моим данным, %s (требуется подтверждение)", answer);
    } else {
...
static int inference_swarm_vote(const char *query,
                                const KolibriFormulaPool *local_pool,
                                char *consensus_answer, size_t consensus_size) {
    if (!query || !local_pool || !consensus_answer || consensus_size == 0) return -1;
...
static int inference_export_formula(const KolibriFormula *formula,
                                    char *export_buf, size_t export_size) {
    if (!formula || !export_buf || export_size == 0) return -1;

    /* Сериализуем формулу для экспорта в рой */
    snprintf(export_buf, export_size,
             "FORMULA:fitness=%.4f:gene_len=%zu",
             formula->fitness, formula->gene.length);
    return 0;
}

/* ---------- #20. Chunked retrieval ---------- */

static int inference_chunked_retrieve(const char *query,
                                      const KolibriFormulaPool *pool,
                                      char *output, size_t output_size,
                                      size_t chunk_size) {
    if (!query || !pool || !output || output_size == 0 || chunk_size == 0) return -1;
...
static int inference_verify_answer(const char *answer,
                                   KolibriGenome *genome) {
    if (!answer || !genome) return 0;
...
static double inference_genome_confidence(uint64_t block_index) {
    if (block_index > 100) return 0.85;
    if (block_index > 50) return 0.7;
    if (block_index > 10) return 0.5;
    return 0.3;
}
