...
int ke_stimulus_queue(KolibriEventLoop *loop, const char *event_type, const char *payload, size_t payload_len,
                      KeStimulusPriority priority, const char *source, uint64_t *out_id) {
    if (!loop || !event_type) {
        return -2;
    }
...
int ke_stimulus_dequeue(KolibriEventLoop *loop, KolibriStimulus *out_stimulus, int timeout_ms) {
    ...
    while (loop->queue_count == 0) {
        if (timeout_ms == 0) {
            ...
            int ret = pthread_cond_timedwait(&loop->queue_not_empty, &loop->queue_mutex, &abstime);
            if (ret == ETIMEDOUT) {
                pthread_mutex_unlock(&loop->queue_mutex);
                return 1; /* Таймаут */
            }
...
int ke_reactor_init(KolibriRuleReactor *reactor, KolibriGenome *genome, KolibriScript *script) {
    if (!reactor) {
        return -1;
    }
...
int ke_reactor_add_rule(KolibriRuleReactor *reactor, const char *name, const char *event_pattern, KeRuleHandler handler,
                        void *user_data) {
...
int ke_reactor_set_rule_active(KolibriRuleReactor *reactor, size_t rule_index, bool active) {
    if (!reactor) {
        return -1;
    }
...
int ke_reactor_tick(KolibriRuleReactor *reactor) {
    ...
    if (any_matched && reactor->genome && commit_success) {
        ReasonBlock block;
        int append_ret = kg_append(reactor->genome, stimulus.event_type, stimulus.payload, &block);
        if (append_ret == 0) {
    ...
    while (1) {
        ...
        if (state == KE_LOOP_PAUSED) {
            ...
            while (atomic_load(&reactor->event_loop.state) == KE_LOOP_PAUSED) {
                pthread_cond_wait(&reactor->event_loop.state_changed, &reactor->event_loop.state_mutex);
            }
        ...
        KolibriStimulus stimulus;
        int ret = ke_stimulus_dequeue(&reactor->event_loop, &stimulus, reactor->tick_interval_ms);
...
int ke_reactor_run_async(KolibriRuleReactor *reactor) {
    ...
    if (pthread_create(&reactor->worker_thread, NULL, ke_reactor_worker, reactor) != 0) {
...
int ke_reactor_pause(KolibriRuleReactor *reactor) {
    ...
    if (!atomic_compare_exchange_strong(&reactor->event_loop.state, &expected, KE_LOOP_PAUSED)) {
        return -1;
    }
...
int ke_reactor_resume(KolibriRuleReactor *reactor) {
    ...
    if (!atomic_compare_exchange_strong(&reactor->event_loop.state, &expected, KE_LOOP_RUNNING)) {
        return -1;
    }
...
int ke_wait_genome_commit(KolibriRuleReactor *reactor, uint64_t stimulus_id, uint32_t timeout_ms) {
    if (!reactor) {
        return -1;
    }
    ...
    if (timeout_ms == 0) {
        ...
        while (reactor->last_committed_id < stimulus_id) {
            int ret = pthread_cond_timedwait(&reactor->commit_done, &reactor->commit_mutex, &abstime);
            if (ret == ETIMEDOUT) {
                pthread_mutex_unlock(&reactor->commit_mutex);
                return 1; /* Таймаут */
            }
...
int ke_wait_all_commits(KolibriRuleReactor *reactor, uint64_t up_to_id, uint32_t timeout_ms) {
