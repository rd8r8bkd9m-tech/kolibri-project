...
def _find_library():
    ...
    raise RuntimeError(
        "Could not find libkolibri_core library. Please build the project first."
    )
...
class CompressionStats:
    ...
    def __repr__(self):
        return (
            f"CompressionStats(ratio={self.compression_ratio:.2f}x, "
...
_lib.kolibri_detect_file_type.argtypes = [
...
def compress(
    data: bytes, methods: int = CompressionMethod.ALL
...
def decompress(data: bytes) -> Tuple[bytes, CompressionStats]:
    ...
    if ret != 0:
        raise ValueError(
            "Decompression failed (possibly corrupt data or checksum mismatch)"
        )
...
# File operations
def compress_file(
    input_path: str, output_path: str, methods: int = CompressionMethod.ALL
) -> CompressionStats:
    """
    Compress a file
