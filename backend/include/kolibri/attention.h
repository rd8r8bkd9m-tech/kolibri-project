...
void kat_gqa_expand_kv(const float *kv, int num_kv_heads, int num_q_heads,
                       int head_dim, float *out);

/* --- SwiGLU утилиты --- */

/** SwiGLU активация: silu(x * w3) * (x * w1 + b1) */
void kat_swiglu(const float *x, const float *w1, const float *b1,
                const float *w3, float *out, int rows, int in_dim, int out_dim);
