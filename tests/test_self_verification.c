...
void test_simple_verification_agree(void) {
    ...
    int ret = kolibri_sv_verify_answer(
    ...
    assert(report.num_methods_used >= 3);
    assert(report.agreement == 1);
    assert(report.final_confidence > 0.7);
    assert(report.verification_passed == 1);
...
void test_verification_accuracy(void) {
    ...
    for (int i = 0; i < total; i++) {
        ...
        printf("  Query %d: confidence=%.2f, passed=%s\n",
               ...
               report.verification_passed > 0 ? "YES" : "NO");

        if (report.verification_passed > 0) {
            passed++;
        }
    }
    ...
    /* При симуляции все должны проходить */
    assert(accuracy >= 80.0);  /* Допускаем небольшой запас */
