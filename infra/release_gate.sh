...
usage() {
    cat <<'USAGE'
Usage: ./infra/release_gate.sh <target>
...
ctest_release_gate_regex='test_kolibri_http_server_api|test_kolibri_http_stream_api|test_kolibri_http_phase1_benchmark|test_semantic|test_phoneme|test_context|test_corpus|test_generation|test_compress|test_formula_logic|test_logical_memory|test_inference|test_fractal_memory|test_attention|test_math_solver|test_reasoning_engine|test_intent_classifier|test_encoding_pipeline|test_world_model|test_auto_learn'
...
run_native() {
    ...
    python3 "$repo_root/infra/check_ctest_inventory.py" --build-dir "$build_dir"
...
run_wasm() {
    ...
    "$repo_root/infra/build_wasm.sh"
}

run_frontend() {
    log_step "running frontend smoke contracts"
    (cd "$repo_root" && npm run test --prefix web)

    log_step "running frontend typecheck"
    (cd "$repo_root" && npm run lint --prefix web)

    log_step "running frontend production build"
    (cd "$repo_root" && npm run build --prefix web)
}
