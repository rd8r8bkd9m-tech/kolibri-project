...
def test_fetch_exchange_rate_answer_for_bitcoin_price(monkeypatch):
    from backend.service.realtime_lookup import fetch_exchange_rate_answer

    def fake_get(url, params=None, headers=None, timeout=None):
        assert "coingecko" in url
        assert params == {"ids": "bitcoin", "vs_currencies": "usd"}
        return _FakeResponse({"bitcoin": {"usd": 81250.0}})

    monkeypatch.setattr("backend.service.realtime_lookup.requests.get", fake_get)

    answer = fetch_exchange_rate_answer("Какая сейчас цена Bitcoin в долларах?")

    assert answer is not None
    assert "1 BTC" in answer
