...
def test_profile_memory_commands(engine):
    ...
    assert who_am_i["response"] == "Вас зовут Алексей."
    ...
    assert "что я люблю" not in remember_fact["response"].lower()
    assert "запомнил:" in remember_fact["response"].lower()
...
def test_profile_memory_query_returns_clean_personal_statement(engine):
    """Личный факт должен возвращаться как нормальный ответ, а не как сырая служебная запись."""
    conv = "profile-memory-query-clean-001"
    engine.chat("Запомни, что мой любимый язык — Rust", conversation_id=conv)

    result = engine.chat("Какой мой любимый язык?", conversation_id=conv)

    assert result["method"] == "profile-memory-query"
    assert "ваш любимый язык — Rust" in result["response"]
    assert "что мой любимый язык" not in result["response"]


def test_remember_fact_ack_uses_clean_personal_form(engine):
    """Подтверждение записи личного факта должно звучать как пользовательский ответ, а не как сырой лог."""
    conv = "profile-memory-ack-clean-001"
    result = engine.chat("Запомни, что мой любимый язык — Rust", conversation_id=conv)

    assert result["method"] == "remember-fact"
    assert "ваш любимый язык — Rust" in result["response"]
    assert "что мой любимый язык" not in result["response"]


def test_followup_why_after_personal_fact_uses_profile_memory(engine):
    """Follow-up 'почему' после личного факта должен разворачиваться из памяти пользователя."""
    conv = "profile-memory-followup-why-001"
    engine.chat("Запомни, что мой любимый язык — Rust", conversation_id=conv)
    engine.chat("Какой мой любимый язык?", conversation_id=conv)

    result = engine.chat("Почему он мне может подходить?", conversation_id=conv)

    assert result["method"] == "profile-memory-query"
    assert "rust" in result["response"].lower()
    assert (
        "контроль над памятью" in result["response"].lower()
        or "производительност" in result["response"].lower()
        or "надёжност" in result["response"].lower()
    )
...
def test_greeting_with_directive_is_not_downgraded_to_fact_ack(engine):
    """Обычная беседа не должна превращаться в служебное подтверждение памяти."""
    result = engine.chat("Привет. Ответь одним коротким предложением.", conversation_id="greeting-directive-001")
    assert result["method"] == "greeting"
    assert "зафиксировал в контексте" not in result["response"].lower()
...
def test_followup_only_on_plain_fact_returns_local_context_fast(engine):
    """Follow-up по локальному факту не должен уходить во внешний lookup и зависать на summary."""
    conv = "dialog-followup-only-fact-001"
    engine.chat("В проекте используется порт 8001 для API.", conversation_id=conv)
    result = engine.chat("А подробнее", conversation_id=conv)
    assert result["method"] == "dialog-context"
    assert "8001" in result["response"]
    assert "контексту текущего диалога" in result["response"].lower()
...
def test_crypto_price_queries_prefer_realtime_lookup(engine, monkeypatch: pytest.MonkeyPatch):
    """Криптовалютные price-запросы должны идти через live exchange lookup."""
    def fake_fetch_exchange_rate_answer(query: str, *, timeout: float = 5.0) -> str | None:
        return "По цене на 2026-04-11 10:00 UTC, 1 BTC ≈ 81250 USD."

    monkeypatch.setattr("backend.service.ai_engine.fetch_exchange_rate_answer", fake_fetch_exchange_rate_answer)

    result = engine.chat("Какая сейчас цена Bitcoin в долларах?", conversation_id="btc-price-001")
...
def test_current_reference_queries_prefer_web_augment(engine, monkeypatch: pytest.MonkeyPatch):
    """Актуальные должности должны идти в web augment, а не в общую справочную summary."""
    monkeypatch.setattr(
        "backend.service.ai_engine.fetch_reference_answer",
        lambda query, *, timeout=6.0: "Президент США — глава государства и исполнительной власти США.",
    )
    monkeypatch.setattr(
        "backend.service.ai_engine.search_quick",
        lambda query, max_urls=5, include_bing_fallback=False, timeout=4: [
            {
                "source": "duckduckgo",
                "title": "President Donald J. Trump - The White House",
                "snippet": "Learn about President Trump's achievements, biography, and plans for his second term in office.",
                "url": "https://www.whitehouse.gov/administration/president-trump/",
            }
        ],
    )

    result = engine.chat("Кто сейчас президент США?", conversation_id="current-reference-001")

    assert result["method"] == "web-reference"
    assert "Donald" in result["response"]
    assert "Trump" in result["response"]


def test_composite_reference_query_combines_current_office_and_capital(engine, monkeypatch: pytest.MonkeyPatch):
    engine._response_cache.clear()
    monkeypatch.setattr(
        engine,
        "_try_current_reference_answer",
        lambda message: "Сейчас президент США — Donald J. Trump.",
    )

    result = engine.chat(
        "Кто сейчас президент США и какая столица Франции?",
        conversation_id="current-reference-composite-001",
    )

    assert result["method"] == "web-reference"
    assert "Donald J. Trump" in result["response"]
    assert "Париж" in result["response"]


def test_composite_reference_query_combines_btc_and_eurusd(engine, monkeypatch: pytest.MonkeyPatch):
    engine._response_cache.clear()
    def fake_fetch_exchange_rate_answer(query: str, *, timeout: float = 5.0) -> str | None:
        low = query.lower()
        if "bitcoin" in low or "btc" in low:
            return "По цене на 2026-04-11 10:00 UTC, 1 BTC ≈ 81250 USD."
        if "евро" in low or "eur" in low:
            return "По курсу на Fri, 11 Apr 2026 10:00 UTC, 1 EUR ≈ 1.09 USD."
        return None

    monkeypatch.setattr("backend.service.ai_engine.fetch_exchange_rate_answer", fake_fetch_exchange_rate_answer)

    result = engine.chat(
        "Сравни текущую цену Bitcoin и курс евро к доллару в одном ответе.",
        conversation_id="composite-rate-001",
    )

    assert result["method"] == "web-reference"
    assert "BTC" in result["response"]
    assert "EUR" in result["response"]
    assert "USD" in result["response"]
...
def test_logic_solver_handles_multi_step_syllogism_without_identity_route(engine):
    """Многошаговый силлогизм не должен ошибочно уходить в identity-intent."""
    conv = "logic-syllogism-chain-001"
    result = engine.chat(
        "Все млекопитающие — животные. Все животные смертны. Кит — млекопитающее. Какой вывод?",
        conversation_id=conv,
    )
    assert result["method"] == "logic-solver"
    text = result["response"].lower()
...
def test_logic_solver_handles_transitive_reasoning_question(engine):
    engine._response_cache.clear()
    result = engine.chat(
        "Если все инженеры умеют решать задачи, а все, кто умеет решать задачи, полезны команде, можно ли сделать вывод, что инженеры полезны команде?",
        conversation_id="logic-transitive-001",
    )

    assert result["method"] == "logic-solver"
    text = result["response"].lower()
    assert "инжен" in text
    assert "полез" in text


def test_article_generation_uses_reference_fallback_when_local_knowledge_is_sparse(
    engine,
    monkeypatch: pytest.MonkeyPatch,
):
    """Статья должна собираться из reference fallback, даже если локальный RAG и web research пусты."""
    monkeypatch.setattr(engine.rag, "query", lambda *args, **kwargs: {})
    monkeypatch.setattr(engine, "_search_graph_by_topic", lambda *args, **kwargs: [])
    monkeypatch.setattr(
        engine.web_research,
        "research_and_learn",
    ...
    monkeypatch.setattr(
        "backend.service.ai_engine.fetch_reference_answer",
        lambda query, *, timeout=4.2: "Римское право — историческая правовая система Древнего Рима, оказавшая огромное влияние на европейскую юриспруденцию.",
    )

    result = engine.chat("Напиши статью про римское право", conversation_id="article-fallback-001")

    assert result["method"] == "article-generation"
    assert "Краткое введение" in result["response"]
    assert "римское право" in result["response"].lower()


def test_longform_note_request_routes_to_article_generation(engine, monkeypatch: pytest.MonkeyPatch):
    engine._response_cache.clear()
    monkeypatch.setattr(engine.rag, "query", lambda *args, **kwargs: {})
    monkeypatch.setattr(engine, "_search_graph_by_topic", lambda *args, **kwargs: [])
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "backend.service.ai_engine.fetch_reference_answer",
        lambda query, *, timeout=4.2: "Римское право — правовая система Древнего Рима, оказавшая большое влияние на европейскую юриспруденцию.",
    )

    result = engine.chat(
        "Напиши развёрнутую заметку про римское право с кратким введением, 3 ключевыми аспектами и выводом.",
        conversation_id="article-longform-001",
    )

    assert result["method"] == "article-generation"
    assert "Краткое введение" in result["response"]
    assert "Ключевые аспекты" in result["response"]


def test_longform_article_uses_requested_outline_sections(engine, monkeypatch: pytest.MonkeyPatch):
    engine._response_cache.clear()
    monkeypatch.setattr(engine.rag, "query", lambda *args, **kwargs: {})
    monkeypatch.setattr(engine, "_search_graph_by_topic", lambda *args, **kwargs: [])
    monkeypatch.setattr(
        engine.web_research,
        "research_and_learn",
        lambda *args, **kwargs: {"combined_text": "", "sources": []},
    )

    def fake_fetch_reference_answer(query: str, *, timeout: float = 4.2) -> str | None:
        low = query.lower()
        if "история" in low:
            return "Римское право складывалось от ранних обычаев и законов XII таблиц до классической юриспруденции и поздней кодификации."
        if "институты" in low:
            return "К ключевым институтам римского права обычно относят право собственности, обязательства и процессуальные формы защиты прав."
        if "современность" in low or "современное" in low:
            return "Римское право повлияло на структуру гражданского права, договорные конструкции и юридическую технику в современных правопорядках."
        return "Римское право — правовая система Древнего Рима, оказавшая большое влияние на европейскую юриспруденцию."

    monkeypatch.setattr("backend.service.ai_engine.fetch_reference_answer", fake_fetch_reference_answer)

    result = engine.chat(
        "Напиши развёрнутую статью про римское право: введение, исторический контекст, 3 ключевых института, современное влияние и вывод.",
        conversation_id="article-outline-001",
    )

    assert result["method"] == "article-generation"
    assert "## Введение" in result["response"] or "## Краткое введение" in result["response"]
    assert "## Исторический контекст" in result["response"]
    assert "## 3 ключевых института" in result["response"]
    assert "## Современное влияние" in result["response"]
    assert "## Короткий вывод" in result["response"]


def test_longform_article_roman_law_uses_section_fallbacks(engine, monkeypatch: pytest.MonkeyPatch):
    engine._response_cache.clear()
    monkeypatch.setattr(engine.rag, "query", lambda *args, **kwargs: {})
    monkeypatch.setattr(engine, "_search_graph_by_topic", lambda *args, **kwargs: [])
    monkeypatch.setattr(
    ...
    def fake_fetch_reference_answer(query: str, *, timeout: float = 4.2) -> str | None:
        low = query.lower()
        if "институт" in low:
            return "К теме иногда относят романо-германскую правовую семью."
        return "Римское право — правовая система, существовавшая в Древнем Риме и в Византии."

    monkeypatch.setattr("backend.service.ai_engine.fetch_reference_answer", fake_fetch_reference_answer)

    result = engine.chat(
        "Напиши развёрнутую статью про римское право: введение, исторический контекст, 3 ключевых института, современное влияние и вывод.",
        conversation_id="article-outline-roman-law-001",
    )

    assert result["method"] == "article-generation"
    assert "главных источников европейской правовой традиции" in result["response"]
    assert "Право собственности" in result["response"]
    assert "Обязательства и договоры" in result["response"]
...
def test_logic_solver_handles_conditional_property_inference(engine):
    engine._response_cache.clear()
    result = engine.chat(
        "Если все проекты с воспроизводимыми тестами легче сопровождать, а Kolibri стремится к воспроизводимым тестам, можно ли заключить, что Kolibri легче сопровождать? Объясни ход вывода.",
        conversation_id="logic-conditional-property-001",
    )

    assert result["method"] == "logic-solver"
    text = result["response"].lower()
    assert "строгий вывод" in text or "нужна более сильная посылка" in text
    assert "kolibri" in text


def test_logic_solver_handles_choice_implication_puzzle(engine):
    engine._response_cache.clear()
    result = engine.chat(
        "У Анны есть три варианта: либо A, либо B, либо C. Если не A, то B. Если B, то не C. Что следует выбрать и почему?",
        conversation_id="logic-choice-001",
    )

    assert result["method"] == "logic-solver"
    text = result["response"]
    assert "A" in text and "B" in text and "C" in text
    assert "нельзя" in text.lower() or "остаётся" in text.lower()
