...
@pytest.mark.anyio
async def test_conversation_title_stays_on_first_message(client: AsyncClient) -> None:
    """Follow-up запросы не должны перетирать title уже созданного диалога."""
    conv_id = "test-conv-sticky-title-001"
    client_id = "test-client-sticky-title"

    first = await client.post(
        "/api/v1/ai/chat",
        json={"message": "Меня зовут Алексей.", "conversation_id": conv_id, "client_id": client_id},
    )
    assert first.status_code == 200

    second = await client.post(
        "/api/v1/ai/chat",
        json={"message": "Почему он мне может подходить?", "conversation_id": conv_id, "client_id": client_id},
    )
    assert second.status_code == 200

    listing = await client.get("/api/v1/ai/conversations", params={"client_id": client_id})
    assert listing.status_code == 200
    items = listing.json()["items"]
    session = next(item for item in items if item["conversation_id"] == conv_id)
    assert session["title"] == "Меня зовут Алексей."
