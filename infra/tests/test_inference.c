...
static void test_create_destroy(void) {
    TEST_BEGIN("create/destroy context");
    tests_run++;
...
static void test_set_temperature(void) {
    TEST_BEGIN("set temperature (clamped 0–2)");
    tests_run++;
...
static void test_run_direct(void) {
    TEST_BEGIN("run DIRECT inference");
    tests_run++;
...
static void test_run_hybrid(void) {
    TEST_BEGIN("run HYBRID inference");
    tests_run++;
    ...
    assert(rc == 0);
    assert(result.response_length > 0);
    assert(result.step_count >= 1); /* Минимум 1 шаг (direct или formula) */
...
static void test_run_formula(void) {
    TEST_BEGIN("run FORMULA inference");
    tests_run++;
...
static void test_run_formula_russian_memory(void) {
    TEST_BEGIN("run FORMULA inference on russian formula memory");
    tests_run++;
    ...
    KolibriInferenceResult result;
    int rc = kolibri_inference_run(ctx, "что такое ядро kolibri ai", &result);
    assert(rc == 0);
    assert(result.response_length > 0);
    assert(result.formulas_applied >= 1);
    assert(strstr(result.response, "искусственный интеллект") != NULL);
    assert(strstr(result.response, "C-ядром") != NULL || strstr(result.response, "C-ядра") != NULL);
...
static void test_numeric_vote_summary(void) {
    TEST_BEGIN("numeric voting summary in FORMULA inference");
    tests_run++;
...
static void test_stats_accumulation(void) {
    TEST_BEGIN("stats accumulate after queries");
    tests_run++;
...
static void test_single_step(void) {
    TEST_BEGIN("single step inference");
    tests_run++;
...
static void test_chain_of_thought(void) {
    TEST_BEGIN("chain-of-thought strategy");
    tests_run++;
