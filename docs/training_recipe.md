...
3. **Подготовьте собственную обучающую (тренировочную) модель**  
   Создайте сценарий в `training/custom_model.ks` (пример уже добавлен). Он будет объединён с автосгенерированным bootstrap, чтобы зафиксировать собственную обучающую (тренировочную) модель.
