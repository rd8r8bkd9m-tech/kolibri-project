...
def send_relations_to_kolibri_sim(
    relations, examples_path="kolibri_entity_examples.txt"
):
    ...
    try:
        subprocess.run(
            ["./build/kolibri_sim", "tick", "--steps", str(len(relations))], check=False
        )
...
def start_background_entity_collector():
    ...
    t = threading.Thread(
        target=background_entity_relation_collector, args=(root_dir,), daemon=True
    )
    t.start()
