...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pending", type=Path, default=Path("build/knowledge/knowledge_genome.dat")
    )
    parser.add_argument(
        "--approved",
        type=Path,
        default=Path("build/knowledge/approved/knowledge_genome_approved.dat"),
    )
    parser.add_argument("--node-binary", type=Path, default=Path("build/kolibri_node"))
    parser.add_argument(
        "--hash-record",
        type=Path,
        default=Path("build/knowledge/approved/knowledge_genome_approved.hash"),
    )
...
def main() -> int:
    ...
    if current_hash == last_hash and args.approved.exists():
        print(
            f"[sentinel] genome unchanged ({current_hash[:8]}...), skipping promotion"
        )
        return 0
    digest = promote(
        args.pending, args.approved, args.node_binary, args.hmac_inline, args.hmac_file
    )
